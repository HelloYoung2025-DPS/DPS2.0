...
## 一之三、半自动驾驶授权（2026-07-17 用户拍板，对应 §9.1"一次性预先签名的有限范围授权"）

用户预授权在**门禁全绿 + Codex 外审 PASS**双前提下自动合入，边界如下；任何 `FAIL/UNAVAILABLE/越范围/两段式取证缺口` 一律冻结并通知用户，不得自动放行。

- **自动合入范围**：常规批次（M2/M3 大部分内容轨）绿即自动合入。
- **必须停下等用户点头的批次**：信任根批次（R0-B/R0-C/R0-D）、改动 Legacy 79 文件字节基线的批次（M3-2/M3-3）、里程碑收口（M1/M2M3/终局）。
...
此授权是程序性的、可随时口头收回；它不改变"模型不持密钥、不能自批准发布、不能清除冻结"（§9.1）。自动合入用 `gh pr merge`，合入人身份仍是用户账号。
