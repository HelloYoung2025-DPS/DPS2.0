...
```text
用 workflow 对 Docs/RebuildPlan_重构计划书.md §14 Definition of Done 做终局评审：逐条核验十项完成事实，核对当前声明的证据等级与实际取得的可执行门证据一致，产出对照裁决表与残留缺口清单。只审不改。把对照裁决表与残留缺口清单写进审计 PR 的描述（空提交/最小 marker PR，落 PR body、不写受跟踪文件，遵 AGENTS.md 门禁状态不入库），开到 dps2 等我合入。
```
