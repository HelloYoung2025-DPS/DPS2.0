...
范围只做本批次：NEGATIVE 与 SAME_INSTANCE 两种断言的评估器、语料与制品检查逻辑的捆绑包及可再现构建脚本；不可变捆绑包/语料/检查器摘要清单；外部锚定材料（受保护路径锚内容由我落位 + 见证账本创世记录内容）；引导发布自身缺陷时的失败/回滚处置（以新引导发布替换、由本版本授权，见 §12 评估器变更条款）。评估逻辑不得引用候选仓库运行时代码；不改 Tools/ci、不动既有模块、不注册 required check（注册属 T2 收口）。
