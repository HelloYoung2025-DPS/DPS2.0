...
范围只做本批次：NEGATIVE 与 SAME_INSTANCE 两种断言的评估器、语料与制品检查逻辑的捆绑包及可再现构建脚本；不可变捆绑包/语料/检查器摘要清单；**m1b-closure 回执的版本化 schema 与 fail-closed validator**（随评估器捆绑包一并钉定并进摘要清单——composition-gate 用它核验 factory 删除类 diff 的账本回执，缺 schema/validator 则该执法面不存在，T2A 不算完成）；**canonical protected-path inventory**（§12 m1b-closure 条款：BOM authority、rollback/receipt 合同、候选门禁信任根等 M1B 证据覆盖的完整受证路径全集，独立于回执单独锚定，外部锚定+见证账本创世记录随本批一并落位；validator 据此对回执路径集合做逐路径双向相等校验；R0-C/T2B 不得修改该全集，变更须独立治理修订——缺此清单及其 validator 校验逻辑，T2A 同样不算完成）；**运行时转换授权逻辑**（§12 运行时激活耦合条款：owner 转换授权策略 schema+validator、敏感转换一次性授权令牌 schema+validator、运行时 challenge 签发/消费协议实现，policy-binding〔策略哈希+版本号+control 记录哈希〕的签发与提交时原子核验逻辑，以及 policy-revocation TOCTOU、oracle-滥用、克隆、中继四类负测，与 composition-gate 评估器同一捆绑包一并交付、外部锚定、进见证账本创世记录，摘要同为本批与 M4 的显式硬退出条件——R0-C、M4 均不得交付或改动该逻辑；缺此逻辑及其负测，T2A 同样不算完成）；外部锚定材料（受保护路径锚内容由我落位 + 见证账本创世记录内容）；引导发布自身缺陷时的失败/回滚处置（常规=以新引导发布替换、由本版本授权；恢复=owner 恢复权威路径，见 §12 评估器变更与恢复条款），并在本批一并锚定 owner 恢复权威材料（独立恢复密钥指纹/流程声明）。评估逻辑不得引用候选仓库运行时代码；不改 Tools/ci、不动既有模块、不注册 required check（注册属 T2 的合入前阶段、由 owner 在冻结窗口执行）。
