...
- **R0 段严格串行**：R0-B(T1) → 评估器引导发布(T2A) → R0-C(T2) → T3-prep(删除候选构造，禁止合入) → R0-D 过渡清单治理批次(T2B，从该候选取摘要) → R0-D 合入(T3-merge) → M1 收口审计(T4)。（T2 施工可先行，但其收口硬前置 = T2A 已由 owner 合入并完成外部锚定；T3-merge 硬前置 = T2B 清单已 owner 授权、外部锚定并并入回执，且合入对象与 T2B 所据候选逐路径一致。）
...
开工前先核对 dps2 远程已合入 PR，判定三态：①前序批次（R0-B）未合入→停下告诉我；②本批已合入，**且其合入本身可证已受门保护**（合入对象在生效 ruleset 下取得来源绑定的 NEGATIVE PASS 与 ledger-witness PASS，即双 check 在合入前已注册且来源绑定），但收口后续项（注册后冒充负测、policy 吊销提交负测、§4.5 clean 证据、m1b-closure 账本回执任一缺失）未完成→不换模板，从缺失项断点续做收口（以账本与平台实际状态为准）；③本批已合入且收口回执齐全可核验→告诉我该用哪个模板；④本批已合入**但无法证明其合入受门保护**（双 check 未在合入前注册\/未来源绑定\/合入对象无对应 PASS）→**禁止事后补收口**（事后注册与回执无法证明当时那次合入被保护，等于追认一次无门合入），停下报我走 revert\/重建 + 重新走带门合入。范围只做本批次，退出条件以 §10 M1B 行为准。上一批证据以 dps2 最新已合入 PR 及其门禁/外审记录为准，自行获取；若上一合入批次触碰候选门禁信任根且尚无合入后正式 clean 证据，先在当前 HEAD 以 --base <其合入提交> 按 §4.5 第 2 步补跑门禁取证，再开工本批。
注意：本批触碰候选门禁信任根（Tools/ci），按 §4.5 两段式取证；Release BOM 由仓外 signer 签发，模型与候选代码不得持签名私钥。**required 注册在本批合入之前**（合入本身必须已被门覆盖，收口后补注册会留下无门合入窗口）：T2A 完成并出齐双 App 可运转证据后、本批进入合入前，由我在 §12 冻结窗口内把永久 required check 组绑定 ruleset：主门 `release-binding.composition-gate` 与独立 verifier `release-binding.ledger-witness`（§12 release-binding-composition 行含 PASS-账本配对条款；当前阶段主门断言=可执行证明生产 publish/dispatch 入口实际不可用）；注册完成前本批不得进入合入，本批合入对象自身必须取得 NEGATIVE PASS 与 ledger-witness PASS——**本批自身合入 PASS 的前提仅为下述①一条自举控制态记录的创建+finalization（§12 PRE_M1B_CLOSURE 条款），否则按 fail-closed 规则两次检查都会失败，此步骤无管理员旁路；下述②evidence-anchor 记录不门槛本批自身的合入 PASS，仅门槛本批合入落地之后另一次 §4.5 空提交证据锚 PR 自身的 PASS——两条记录各自独立生效、互不构成对方前提，不存在"本批合入需要先有 evidence-anchor、而 evidence-anchor 又需要本批先合入"这类循环依赖**：①**bootstrap-candidate**——本批 PR 达到候选合入 head 后，由我在冻结窗口内让 App 向 `control` 追加并 finalize 一条 bootstrap-candidate 记录，绑定本批精确 base SHA/PR head SHA/结果树哈希；finalize 落账后，composition-gate 与 ledger-witness 才能对该精确 head 用 PRE_M1B_CLOSURE 哨兵签出 PASS（其余对象仍 FAIL）——此步骤停下等我在冻结窗口内操作，完成后告诉我继续，随后本批即可凭该 PASS 进入合入；②**evidence-anchor**——本批合入落地后、§4.5 空提交证据锚 PR 提交前，由我在冻结窗口内让 App 向 `control` 追加并 finalize 一条 evidence-anchor 记录，绑定 R0-C 合入提交 SHA；finalize 落账后，仅"父提交=R0-C 合入提交且结果树与之相同"的空 diff 子提交才能取得双 PASS——此步骤同样停下等我操作。两处均须在 PR 描述附冻结窗口序列证据（锁定/排空/pending/activation/finalization/解锁记录哈希），作为"合入受门保护"的可核验凭据。**端到端验证**：依序跑通①②后确认 R0-C 候选凭①、空提交锚凭②各自取得双 PASS、且过程无管理员旁路（管理员绕过会使"合入受门保护"的证明失效，等同④态）。评估器本体由前置的独立引导治理发布供给并外部锚定——收口前核验其四件套（owner 合入提交、外部摘要锚、见证账本创世记录、独立外审证据），缺一即 R0-C 不得收口；本批不得交付或改动评估器，只交付被测负向状态。合入后收口阶段，在生效 ruleset（已含 integration ID 来源绑定）上实测签发者冒充负测（主门与 verifier 各自执行）：预期 App 的报告可满足对应 required check，其他 App、用户、commit status API 与 Actions workflow 的同名结果均不能满足；另实测配对拦截负例：主门 PASS 存在而账本无配对 conclusion 记录时，verifier 必须 FAIL、对象无法合入；证据绑定 ruleset 标识、修订版本与本批精确合入提交——该负测是 §10 M1B 硬退出条件，缺它 M1B 不得关闭。**本批还须自行执行 policy 吊销提交负测**（从 T2A 移交，见 §12）：在自建的 `control-plane-host` 提交路径上，取一枚在策略 P1 下签出的合法 transcript，冻结窗口内将 P1 撤销/收窄为 P2 并 finalize，用该 P1 transcript 提交转换（覆盖 revoke/rollback/promote 等敏感转换类型），必须被原子拒绝、不消费 challenge、不改 binding；证据存档——该负测同为 §10 M1B 硬退出条件，缺它 M1B 不得关闭。
完成后：跑 required 门禁、按 Docs/Operations/ExternalReview_外审机制.md 走批次收尾 Codex 外审、开 PR 到 dps2，然后停下等我批准。若本批触碰候选门禁信任根：我确认合入后不结束会话，按 §4.5 第 2 步在合入提交的后继提交上取正式 clean 证据（main 暂无后继提交时，先开一个空提交 PR 作证据锚、经我合入后再取证），结果贴回本批 PR。合入后收口阶段还必须完成注册后冒充负测（绑定本批精确合入提交与生效 ruleset 标识/修订，见上），然后由专用 App 按 T2A 钉定的 schema 向**见证账本追加签名 m1b-closure 回执记录**——该落账属改变合入资格的记录，必须按 §12 冻结协议执行（锁定-排空-fencing-pending/activation/finalization-解锁-重入队，序列证据存档；中途失败走 abort/恢复语义，禁止无冻结直接追加）（绑定本批精确合入提交 SHA、ruleset 标识/修订、integration ID、冒充负测证据摘要、**policy 吊销提交负测证据摘要**（revoke/rollback/promote 三类各自独立的"P1 撤销后提交被拒"实测记录，见 §12 与 m1b-closure schema——validator 核验 outcome=拒绝、目标提交/部署身份匹配本回执、三类不缺一，任一不满足回执无效）、§4.5 clean 证据摘要、T2A 四件套锚点摘要、App 可运转与隔离证据摘要及精确部署/配置身份标识——账本记录是权威回执，composition-gate 据它放行 T3，validator 会逐项解引用核验隔离与运转证据）；再把该账本记录哈希与人读摘要贴回本批 PR（PR 内容只是索引，不充当执法对象）。账本回执落账且字段可解引用，本批才算关闭。
```
...
开工前核对 dps2 已合入 PR，判定三态：①R0-B(T1) 未合入→停；②本批已合入但硬退出条件未全数满足（外部锚定、账本创世记录、主/verifier App 可运转与隔离证据、评估器签名服务部署与公钥/度量锚定、密钥释放隔离与全沦陷负测、下列各项任一缺失或不可核验）→不得回 T2，从缺失项断点续做直至全部证据齐全；③已合入且硬退出条件逐项可核验→告诉我直接回 T2 收口。开工前还必须完成平台能力预检（§12 平台能力硬前置）：目标仓库已迁移至组织名下、merge queue 可在受保护分支实际启用、审计日志经真实锁定/解锁演练证明锁会话可被机器唯一归属（含相邻/并发锁操作下无歧义；只证"可读取"不合格，见 §12 锁会话标识定义）——个人账户仓库不支持 merge queue，预检任一不通过即停下报我（仓库迁移由我执行），不得以手工合入替代协议语义开工。
范围只做本批次：NEGATIVE 与 SAME_INSTANCE 两种断言的评估器、语料与制品检查逻辑的捆绑包及可再现构建脚本；不可变捆绑包/语料/检查器摘要清单；**m1b-closure 回执的版本化 schema 与 fail-closed validator**（随评估器捆绑包一并钉定并进摘要清单——composition-gate 用它核验 factory 删除类 diff 的账本回执，缺 schema/validator 则该执法面不存在，T2A 不算完成）；**canonical protected-path inventory**（§12 m1b-closure 条款：BOM authority、rollback/receipt 合同、候选门禁信任根等 M1B 证据覆盖的完整受证路径全集，独立于回执单独锚定，外部锚定+见证账本创世记录随本批一并落位；validator 据此对回执路径集合做逐路径双向相等校验；R0-C/T2B 不得修改该全集，变更须独立治理修订——缺此清单及其 validator 校验逻辑，T2A 同样不算完成）；**运行时转换授权逻辑（评估器/见证账本侧，边界见 §12）**（owner 转换授权策略 schema+validator、敏感转换一次性授权令牌 schema+validator、运行时 challenge 签发/消费协议实现，policy-binding〔策略哈希+版本号+control 记录哈希〕的签发侧生成逻辑，以及**评估器侧**的 oracle-滥用/克隆/中继负测与**评估器拒签负测**（本地所见策略头落后于 control 最新 finalize 记录时拒绝签发新 transcript）——以上均只需评估器+见证账本+`control` 分支即可自证，不依赖候选仓库任何运行代码。**"已签出合法 transcript 在策略吊销后提交仍被拒"的提交路径负测不在本批**：`control-plane-host` 模块目录虽随 458f9bd 快照已随仓库落地，但 `active.release.binding/v1` 本身及其提交路径是 R0-C 的施工产出、本批阶段并未建成——把该负测留作本批硬退出条件会形成"依赖尚未存在的 R0-C 产出"的阶段循环，故移交 R0-C 自身退出条件（见 T2 模板）；本批交付范围止于"签出/校验规则本身"，不含把该核验接入具体 authority 事务代码（那是 R0-C 在 `control-plane-host` 内按本批锚定 schema 实现的自身消费逻辑；生产多实例下的策略头传播与陈旧实例 fencing 是 M4 范围，见 §12）；与 composition-gate 评估器同一捆绑包一并交付、外部锚定、进见证账本创世记录，摘要同为本批与 M4 的显式硬退出条件；缺此逻辑及其负测，T2A 同样不算完成）；外部锚定材料（受保护路径锚内容由我落位 + 见证账本创世记录内容）；引导发布自身缺陷时的失败/回滚处置（常规=以新引导发布替换、由本版本授权；恢复=owner 恢复权威路径，见 §12 评估器变更与恢复条款），并在本批一并锚定 owner 恢复权威材料（独立恢复密钥指纹/流程声明）。评估逻辑不得引用候选仓库运行时代码；不改 Tools/ci、不动既有模块、不注册 required check（注册属 T2 的合入前阶段、由 owner 在冻结窗口执行）。
完成后：对精确 diff 走独立 Codex adversarial review 至明确 PASS；开独立 PR 到 dps2，PR 记录捆绑包摘要清单与外审作业标识；停下等我亲自合入并完成外部锚定（受保护路径 + 见证账本创世记录）。**硬退出条件还包括专用 App 的可运转证据（缺一 T2A 不算完成、不得回 T2；主 App 与独立 verifier App `release-binding.ledger-witness` 两者各自满足，verifier 独立部署/凭据保管/度量锚定、对 control/conclusions 只读、仅对 challenges 分支有窄域追加/CAS 权（由分支定向 ruleset 执法：challenges ruleset 仅许 verifier、control/conclusions ruleset 仅许主 App，非单一共享 ruleset；写其他分支必须失败，跨分支写负测实测，各 ruleset ID/修订入证据））**：专用 GitHub App 已仓外部署并安装到本仓库、权限最小化、运行环境候选不可写、**签发凭据与候选执行面隔离已实证**（候选评估在无 App 凭据/无 Checks 与见证仓访问/无宿主敏感挂载/默认无出网的临时沙箱内执行，签发由独立服务经单向认证结果通道完成，候选读凭据/调 Checks API/篡改结果通道/逃逸 worker 四项攻击测试均失败，见 §12）、具备 merge-group 触发能力、主 App 已成功签发至少一次（非 required 的）`release-binding.composition-gate` check、verifier App 已成功签发至少一次（非 required 的）`release-binding.ledger-witness` check、两者独立 integration ID 各自经核验记录；**独立评估器签名服务已部署并锚定**（§12 conclusion 授权因子分离）：评估器服务独立度量部署、transcript 验证公钥与评估器度量随捆绑包摘要一并外部锚定、保管服务的密钥释放隔离实测（transcript 签名密钥仅向评估器度量释放，check 报告与账本 push workload 取钥必须失败）、主签发方全沦陷伪造 conclusion 被 verifier 验签拒绝的负测已实测——GitHub ruleset 只有在预期来源 App 已安装且近期提交过该 check 后才能绑定来源，先注册后部署要么死锁要么开放冒充面。T2A 只验证专用 App 能在目标仓库签发该 check；同名冒充负测不在 T2A 执行（ruleset 尚未注册、没有合入接受判定面，此时的"负测"证明不了未来配置正确），移至 T2 完成 required 注册与 integration ID 绑定之后实测（见 T2 模板与 §12/§10 M1B）。全部证据齐全后 T2 方可收口；T2 收口核验四件套+App 可运转证据须绑定本批的精确合入提交。
```
...
开工前先核对 dps2 已合入 PR：M2、M3 未全部合入、或 M2/M3 收口审计（T13）的裁决不是可核验的 PASS（未完成、FAIL/UNAVAILABLE、记录不一致或无法核验、有未决 critical，均属非 PASS，一律停），则停下告诉我；本批次已合入则告诉我该用哪个模板。范围只做本批次：composition root（含 §4.3 交付边界移交的同实例义务：SAME_INSTANCE 评估器由 R0-C 之前的独立引导治理发布钉入专用 App 捆绑包（§12/§16）——开工前先核验该引导发布四件套：owner 合入提交、外部摘要锚、见证账本创世记录、独立外审证据，缺一即停；本批不得携带任何评估器改动（评估器变更须走独立治理发布）；模式切换按 §12 影子先行协议：先以外签影子检查对本批精确 merge-head 跑 SAME_INSTANCE 断言并取得 PASS，然后 owner 才按冻结-排空-追加-重入队协议切 epoch、重入队该已证明的 head 合入——严禁在影子 PASS 之前切棘轮（现行 main 无法满足 SAME_INSTANCE，先切即永久冻仓）；追加 activation 前 App 机器校验锁定后的 main tip 仍等于影子元组的 base SHA，漂移即 abort 不激活——此时在锁内以新 base 重构结果对象、重跑影子检查取得新影子 PASS 后再激活，或解锁重启序列（§12 base 漂移锁内前置校验）；本批只交付被测生产组合制品，由既有评估器判定——policy-approval 与 executor-gateway 由同一 active-binding provider 实例生产组合，断言覆盖设备域五元组 (device_binding_id, release_bom_sha256, generation, token digest, status)、activation/revocation/rollback、restart、并发读、同设备并发写写竞争（activate/activate、activate/revoke、revoke/rollback、重试/重启交错——CAS 单胜者、败者明确冲突、generation 不复用、receipt 对应提交顺序）与并发双设备隔离负例；**§12 运行时转换授权条款要求的策略头一致传播与陈旧实例 fencing**：policy finalize 落账后须原子/一致地传播到本批组装的全部 authority 实例，未收到新策略头的实例在传播完成前 fence 住转换提交，负测覆盖"新策略已生效但陈旧实例仍以旧策略接受旧 transcript"场景（缺此负测本批不算完成）；外部 ruleset 与专用 App 签发者绑定不变，本批附转换测试与证据证明 merge-head 以 attested mode=SAME_INSTANCE 通过，见 §12 release-binding-composition 行）、attempt/receipt/eligibility 合同、session+command 双 300 滚动门、runtime kill switch 与工程 freeze、通知 outbox。退出条件以 §10 M4 行为准（含 kill-notify suite 的未授权清除负例与 `release-binding.composition-gate` 的 same-instance 断言）。上一批证据以 dps2 最新已合入 PR 为准，自行获取；若上一合入批次触碰候选门禁信任根（含 catalog/DAG/compatibility 等合同登记文件）且尚无合入后正式 clean 证据，先在当前 HEAD 以 --base <其合入提交> 按 §4.5 第 2 步补跑门禁取证，再开工本批。
完成后：跑 required 门禁（reliability、kill-notify 与 release-binding.composition-gate）、按 Docs/Operations/ExternalReview_外审机制.md 走批次收尾 Codex 外审、开 PR 到 dps2，然后停下等我批准。若本批触碰候选门禁信任根：我确认合入后不结束会话，按 §4.5 第 2 步在合入提交的后继提交上取正式 clean 证据（main 暂无后继提交时，先开一个空提交 PR 作证据锚、经我合入后再取证），结果贴回本批 PR，完成后本批才算关闭。
```
