...
开工前先核对 dps2 远程已合入 PR，判定三态：①前序批次（R0-B）未合入→停下告诉我；②本批已合入，**且其合入本身可证已受门保护**（合入对象在生效 ruleset 下取得来源绑定的 NEGATIVE PASS 与 ledger-witness PASS，即双 check 在合入前已注册且来源绑定），但收口后续项（注册后冒充负测、§4.5 clean 证据、m1b-closure 账本回执任一缺失）未完成→不换模板，从缺失项断点续做收口（以账本与平台实际状态为准）；③本批已合入且收口回执齐全可核验→告诉我该用哪个模板；④本批已合入**但无法证明其合入受门保护**（双 check 未在合入前注册\/未来源绑定\/合入对象无对应 PASS）→**禁止事后补收口**（事后注册与回执无法证明当时那次合入被保护，等于追认一次无门合入），停下报我走 revert\/重建 + 重新走带门合入。范围只做本批次，退出条件以 §10 M1B 行为准。上一批证据以 dps2 最新已合入 PR 及其门禁/外审记录为准，自行获取；若上一合入批次触碰候选门禁信任根且尚无合入后正式 clean 证据，先在当前 HEAD 以 --base <其合入提交> 按 §4.5 第 2 步补跑门禁取证，再开工本批。
