...
1. **审计模板（T4/T13/T19）是强制门，不是可选项**：审计会话把对照裁决表写进**审计 PR 的描述**（遵 `AGENTS.md`「门禁状态不入库、只落 PR/任务描述」——审计不改产品码，用空提交或最小 marker PR 承载，裁决表落 PR body，**不写受跟踪文件**，否则普通 docs 编辑就能伪造审批状态）。后续施工模板据该**已合入审计 PR 的描述**核验对应审计已完成且无未决 critical；无该 PR 或其中有未决 critical 会拒绝开工。
