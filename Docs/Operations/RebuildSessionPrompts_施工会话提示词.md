...
> 用法: 每批一个会话，完全掌控——按计划书 §16 的**依赖顺序**挑对应 T# 模板，在 `DSP_ZD` 目录开 Claude Code 会话整块粘贴、不改字。**T 编号只是模板索引，不是执行顺序**：R0 段串行、M2 轨 ∥ M3 轨并行、跨轨/探针前置（T15 探针先于 T10、T10 合入后才可开 T8）见下方分段说明；每个模板自带开工前置门，前置未满足会让你停下（例如 T10 会要求先完成 T15、T8 会要求先合入 T10）。
