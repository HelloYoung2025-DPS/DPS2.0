...
开工前核对 dps2 已合入 PR：R0-B(T1) 未合入则停；本批已合入且已锚定则告诉我直接回 T2 收口。开工前还必须完成平台能力预检（§12 平台能力硬前置）：目标仓库已迁移至组织名下、merge queue 可在受保护分支实际启用、审计日志经真实锁定/解锁演练证明锁会话可被机器唯一归属（含相邻/并发锁操作下无歧义；只证"可读取"不合格，见 §12 锁会话标识定义）——个人账户仓库不支持 merge queue，预检任一不通过即停下报我（仓库迁移由我执行），不得以手工合入替代协议语义开工。
