...
1. **审计模板（T4/T13/T19）是强制门，当前是程序性门、非机器防篡改门**：外审裁决写入对应审计 PR 描述以遵 `AGENTS.md:90`（审计不改产品码，用空提交/最小 marker PR 承载，裁决落 PR body、不写受跟踪文件）。该记录**仅是可见的程序性审计记录，不是不可篡改证据、也不是机器放行信号**——PR 描述可被事后编辑，任何描述修改本身都不得产生、恢复或替代 `PASS`。后续施工模板"核验对应审计已完成且无未决 critical"是**提示用户人工核对**（读该已合入审计 PR 的原始外审输出、已审 head/base、裁决及其一致性），不是机器可信判定；缺审计记录、或出现 `FAIL`/`UNAVAILABLE`/记录不一致/无法核验，一律冻结、拒绝开工。将裁决绑定到不可篡改外部 check/签名证据/merge 制品的机器级审计门，明确延期至 **R0 后的 §9.1 专门批次**；本文不宣称当前已具备防篡改审计保证。
...
- **M1C 合入、T4 无未决 critical、且 R0 后『§9.1 安全并行与门禁完整性』专门批次已合入并启用后**，两轨并行、各轨内部按序：M2 轨 T5→T6→T7→T8；M3 轨 T9→T10→T11→T12。（在该专门批次合入前只串行、不开并行 PR、不启用 merge queue——见外审机制 §三-5。）
