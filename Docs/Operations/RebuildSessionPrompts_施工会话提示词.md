...
1. **审计模板（T4/T13/T19）是强制门，不是可选项**：审计会话把对照裁决表作为独立 docs PR 合入 dps2（落 `Docs/Operations/Audits/<里程碑>_收口审计.md`，是审计唯一产物，仍不改产品码、不补证据），作为跨会话可核验的持久记录。后续施工模板据该**已合入审计 PR** 核验对应审计已完成且无未决 critical；无该 PR 或其中有未决 critical 会拒绝开工。
...
- **M1C 合入且 T4 无未决 critical 后**，两轨并行、各轨内部按序：M2 轨 T5→T6→T7→T8；M3 轨 T9→T10→T11→T12。
...
```text
用 workflow 对 Docs/RebuildPlan_重构计划书.md §14 Definition of Done 做终局评审：逐条核验十项完成事实，核对当前声明的证据等级与实际取得的可执行门证据一致，产出对照裁决表与残留缺口清单。只审不改。把对照裁决表与残留缺口清单落成 `Docs/Operations/Audits/终局_DoD审计.md`、作为独立 docs PR 开到 dps2 等我合入（审计唯一产物，不改产品码、不补证据）。
```
