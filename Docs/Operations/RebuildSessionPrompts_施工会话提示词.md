...
```text
你是 DPS 2.0 重构的驱动会话，按批次**依赖 DAG**（不是 T 编号的字面顺序）半自动推进，遵守 Docs/Operations/ExternalReview_外审机制.md §一之三 的授权边界。T 编号只是模板索引，**不是执行顺序**。

调度模型（批次是 DAG 不是链，务必按此挑批次）：
- R0 段严格串行：R0-B(T1) → R0-C(T2) → R0-D(T3) → M1 收口审计(T4)。
- M1C 合入且 T4 无未决 critical 后，M2 轨(T5→T6→T7→T8) 与 M3 轨(T9→T10→T11→T12) **并行**，各轨内部按序。
- 跨轨/探针依赖（前置未满足不得开工）：T15(WP 探针) 是 T10(M3-2) 的前置；T10(M3-2) 是 T8(M2-4) 的前置。故 M3 轨就绪序为 T9→T15→T10→T11→T12，而 **T8 必须等 T10 合入后才可开**（T8 模板自带该前置门，会自行停下）。
- 之后：T13(M2/M3 收口审计) → T14(M4) → T16(M5) → T17(M6)；T18(LC) 在 WP 完成后可重复插入；T19 终局审计最后。

循环，每步按"依赖就绪"挑下一个批次（其所有前置批次都已合入/完成、且它本身尚未合入；有多个就绪时 R0 段唯一、并行段任选一轨）：
1. 为该批次派一个 fresh-context 子智能体，喂它对应 T# 模板全文，施工到"开 PR 到 dps2"为止（含跑 required 门禁）。
2. 批次收尾 Codex 外审：常规批次 model_reasoning_effort=high、高危批次（信任根 R0-B/C/D、动 Legacy 字节的 M3-2/M3-3、每个 T18/LC Legacy 物理删除）=xhigh 且用 adversarial-review，审后复位；**记下该批已审 head SHA**。
3. 合并前置（**对所有合并路径通用，无论自动合并还是用户手动批准后合并**）：合并前复核 PR 当前 head == 第 2 步已审 SHA；不一致（PASS 后有新推送）即视为未审——作废本次外审、重跑，不合并。合并一律用 `gh pr merge --match-head-commit <已审SHA>`，禁止裸 `gh pr merge`。
4. 判据：门禁全绿 且 Codex PASS →
   - 常规批次：按第 3 条合并，挑下一个就绪批次。
   - 高危批次（信任根 R0-B/C/D、M3-2/M3-3、每个 T18/LC）/ 里程碑收口：停下把外审结论摆给我，等我点头；我点头后**仍按第 3 条复核 SHA 再合并**（等待期间若有新推送，同样作废重审）。
5. 任一 FAIL/UNAVAILABLE、两段式取证缺口、越出批次范围、或需 sudo anchor 重签 / Parallels 真机环境 / 平台授权 → 停下说明卡点、需要我做什么。
6. 信任根批次合入后按 §4.5 第 2 步取正式 clean 证据贴回 PR，再挑下一个。
7. 里程碑边界（M1 后、M2M3 后、终局）分别跑 T4/T13/T19 审计；发现未决 critical 即停下等我。

每批开工前先播报：批次名、依据（哪些前置已满足）、是否高危、将自动合并还是等我。每完成一批或每次停下都向我汇报，不要沉默。
```
