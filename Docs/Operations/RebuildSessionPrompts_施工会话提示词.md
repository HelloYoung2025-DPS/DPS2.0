...
开工前先核对 dps2 已合入 PR：M2、M3 未全部合入、或 M2/M3 收口审计（T13）的裁决不是可核验的 PASS（未完成、FAIL/UNAVAILABLE、记录不一致或无法核验、有未决 critical，均属非 PASS，一律停），则停下告诉我；本批次已合入则告诉我该用哪个模板。范围只做本批次：composition root（含 §4.3 交付边界移交的同实例义务：请 owner 先签 rollout epoch 把 `release-binding.composition-gate` 期望模式切到 SAME_INSTANCE，然后交付其断言实现——policy-approval 与 executor-gateway 由同一 active-binding provider 实例生产组合，断言覆盖设备域五元组 (device_binding_id, release_bom_sha256, generation, token digest, status)、activation/revocation/rollback、restart、并发读与并发双设备隔离负例；外部 ruleset 与签发者绑定不变，本批附转换测试与证据证明 merge-head 以 attested mode=SAME_INSTANCE 通过，见 §12 release-binding-composition 行）、attempt/receipt/eligibility 合同、session+command 双 300 滚动门、runtime kill switch 与工程 freeze、通知 outbox。退出条件以 §10 M4 行为准（含 kill-notify suite 的未授权清除负例与 `release-binding.composition-gate` 的 same-instance 断言）。上一批证据以 dps2 最新已合入 PR 为准，自行获取；若上一合入批次触碰候选门禁信任根（含 catalog/DAG/compatibility 等合同登记文件）且尚无合入后正式 clean 证据，先在当前 HEAD 以 --base <其合入提交> 按 §4.5 第 2 步补跑门禁取证，再开工本批。
