...
6. 过渡合入条款（存量红治理期，对第 4-5 条的限定）：在 main 的 required 检查存在已登记 `BASE_PREEXISTING` 存量失败期间，同时满足以下全部条件的批次，可由仓库所有者亲自执行管理员合入，不受第 4 条全绿语义阻断：
   ...
   **本条去向（Owner 三选一；签署机制同 §4.6 前言，属不可逆治理决策，故不预填默认项；任一项签署前，本条按上方现行条文继续执行）**：
   - ☑ (甲) 修前提保留：第一项「已登记存量清单子集」的机器判定，过渡期由 Owner 人工比对已登记存量失败集执行（人工比对不放宽零新增失败标准），`governance/baseline-red-ledger.json` 存量红账本与 `Tools/ci/run_phase0_gate.py` 三态判定（通过 / 已登记存量红 / 新增失败即阻断，由专门批次交付）落地后由账本承接。后果：存量红治理期保留一条可用的过渡合入路径，账本落地前其判定可信度依赖 Owner 人工比对质量。（签署：2026-07-26）
...
1. 合入前，在批次 PR head 上取 required 静态门证据：`.github/workflows/static-ci.yml` 的 required check 即证据来源，其原始证据由 CI 以 artifact 自动留档（`dps-phase0-evidence-<head sha>`，`if: always()` 上传，保留期以该工作流的 `retention-days` 配置为准；artifact 过期后平台上还留下什么见下段，本条不作断言）。触碰信任根不增加任何额外的人工取证步骤；批次以「required check `PASS` + 外审 + 所有者合入」即告关闭，合入即终点，无后续取证动作。

   **保留期届满后的证据状态（如实界定记法，不创设留档任务）**：artifact 过期后其原字节在平台上不再可取。**平台上此后还留下什么，本节不作断言**——截至 2026-07-26 本仓库尚无任何 artifact 到期（最早到期日 2026-08-14），GitHub 侧结论与日志的留存行为在本仓库从未被实测过，不得写成已核实事实。已实测且可复现的是：**required check 对象本身不含任何门禁输出**——本仓库已产出的全部 check suite 上 `output.summary`/`output.text`/`output.title` 一律为 `null`，workflow 也没有任何写 check-run output 的步骤。故即使结论确实长期可查，它按施工提示词库硬规则 1 恰恰就是「空壳」，不构成「真实含门禁输出」的证据——artifact 过期即等于该批次的门禁输出在平台上灭失。届满后审计对该批次的 artifact 引用一律记 `UNAVAILABLE`；若该批次证据已按项目边界另存仓外归档，可在同一条目上附归档位置与逐文件 SHA-256 摘要（仅当摘要可回读校验时）——该附注只让 `UNAVAILABLE` 条目可被追索，不使任何充要条件视为满足、不改变任何门槛；本节不指定归档设施、不指定执行人、不要求任何人去归档。**过期不得被记为通过**——`UNAVAILABLE` 同样是 fail-closed，不满足 T4/T13/T19「写 `PASS` 的充要条件」。本段只界定记法，不产生任何合入后才到期的义务、不依赖任何后续 PR。
...
- ☑ 确认 §4.5 原第 2 步（合入后在后继提交上以 `--base D` 取 clean 候选证据）及其全部派生仪式（证据锚 PR、空提交锚、专职补证会话、收口审计对该步锚点的逐条核验）自 §4.5「生效条件」成立之日起整体废止；废止面向未来，历史记录不改写。（签署：2026-07-26）
...
- **签署前须知（§4.5「残余披露」的就地交叉引用；本须知不增减本项的任何效力）**：本项签署后，触碰信任根批次据以关闭的 required 静态门 `PASS`，是**由候选自身携带的校验器签发的**——`.github/workflows/static-ci.yml` 与它执行的 `Tools/ci/run_phase0_gate.py` 本身都在 `CANDIDATE_TRUST_PATHS` 内，一个削弱校验器的改动可以让被削弱后的校验器为它自己出具 `PASS`。**该残余不随本项签署与否而改变**：本项未签署时回落的原两段式，其第 1 步与第 2 步同样由候选自身携带的代码执行，且第 2 步在本仓历史上从无 CI 执行路径。故本项签署的是「原第 2 步是否废止」，**不是**「自签发问题是否已解决」；后者的机制解法归口为仓库设置层面的配置工作（为 `main` 启用 required status checks、收紧施工凭证），不属于任何施工批次的交付范围。全文、实测依据及其与 `Docs/Operations/RepositoryProtection_仓库保护.md:21` 的正面冲突见 §4.5「残余披露」。另一并如实提示：在施工凭证收紧（Owner 侧阶段性配置工作，与 required checks 同归口）完成前，「所有者本人签署/批准」与执行凭证的区分依赖账号使用纪律——平台记录能证明批准事件存在并绑定精确 head，不能证明按键者身份；此即历史收口文书 B-1/B-2 所定性的同类限制，其机制解法是凭证分离，不是任何文书措辞。本须知只作提示：不新增勾选项、不创设待办、不产生任何合入后才到期的义务或后续 PR 依赖。
...
**合入顺序与前提（本条不断言其已经发生）**：PR-A 须先于本文书所在的 PR-B 合入，且 PR-A 的合入以 Owner 在 PR-A 上给出平台可见、绑定精确 head 的批准为前提。分支拓扑（PR-B 的 base 分支即 PR-A 分支）保证的只是：本文书的内容不经 PR-A 分支即无法抵达 main。**拓扑不阻止的操作是**：把 PR-B 先合入 PR-A 分支，使两份文书折叠为一个 PR 一并送签（实测 PR-B 对其 base 分支随时 MERGEABLE）——该操作会取消「先 A 后 B、分开签署」的边界，与 Owner 决策的签署顺序相抵，故合入执行方不得执行之：PR-B 仅在 PR-A 合入 main、且其 base 已实际改指 main 之后方可合入。**base 改指不是自动的**——实测本仓库 `delete_branch_on_merge` 为 `false`，平台只在 base 分支被删除时才自动改指；故改指由合入执行方显式完成（删除已合入的 PR-A 分支以触发平台改指，或直接编辑 base），并在改指后核验本 PR 的 diff 仅含本文书内容。顺序保证是这条程序纪律加 Owner 的两次分开批准，不是拓扑本身，也不是平台默认行为。若 PR-A 被打回，或其裁决内容偏离「三条均追认」，本项陈述即不成立，须在 PR-B 合入前校正。

**(c) M1 冻结解除条件与 M2 放行声明**

- ☑ 确认 M1 冻结解除条件为下列三项全部成立：(1) 本文书 (a)、(d)、(e) 各项均已签署，且 (b) 所指的 PR-A 历史收口文书已合入；(2) main tip 的 Phase 0 required `overall_status == PASS`（以受信环境一次完整跑为准）；(3) T4 按 §4.5「生效条件」当时成立与否的对应条文重跑，裁决表逐条如实产出，并取得可核验 `PASS`（定义见施工提示词库硬规则 1，不放宽）。**历史批次取证缺口对本项的作用（如实界定，不设折算）**：**无论 (e) 勾哪一支**，至 PR #16 的已合入批次其合入前 required 静态门证据一律据实记 `UNAVAILABLE`（`overall_status == FAIL` 的历史运行据实记 `FAIL`），硬规则 1 中「§4.5 合入前取证证据齐全」一项对这些批次**一律计为不满足**：`RISK_ACCEPTED` 标注不满足该充要条件；(e) 的等价重验记录同样不满足——它跑的是当时 main tip 的树与该批次的用例子集，与这些批次合入前的 head 及其门禁结果不同源，只能说明「该批次的承载用例在今天的 main 上仍然通过」，不能证明其合入前的门禁结果，故只作附于同一条目的补充记录，不折算为该项满足。**由此如实推出**：在本计划书现行条文下，第 (3) 项对 M1 不成立，M1 维持冻结——风险接受关闭的是「是否继续追补历史证据」，等价重验关闭的是「是否再做一次今日等价性说明」，两者都不是解冻条件。任何情况下**不得**为解冻把裁决改写为 `PASS`、不得声称 M1「证据齐全」、不得把 `RISK_ACCEPTED` 或 `REVALIDATED@<CI run ref>` 计为通过项，也不得据此绕过下游 T5/T9/T14 前置门。本文书不预设、也不预先授权任何绕过第 (3) 项的解冻路径；所有者此后若要解冻，只能另行以明示的、绑定精确范围的计划书修订重新界定 T4 对至 PR #16 历史批次的证据要求并自行具名承担——**那是所有者的选项，本文书不产生该义务、不设时限、不依赖任何后续 PR**，不作此修订即维持冻结。（签署：2026-07-26）
- ☑ M2 放行声明：上述三项全部成立后，M2/M3 轨按 §10 依赖顺序放行开工。本项签署本身不构成放行；三项未全部成立前，M2/M3 维持冻结。放行不改变施工提示词库硬规则 3 的档位判定：档位升迁另有触发条件，未升档前工作单位与仪式仍按当时档位执行。（签署：2026-07-26）

**(d) 零 diff 空转 PR 的定性**

- ☑ 确认 PR #7、#9、#11、#12、#16 为已废止的合入后取证义务产生的零 diff 空转 PR，定性为程序遗产：保留于历史、不再补证、不得引用为任何批次的证据。（签署：2026-07-26）

**(e) 历史批次合入前取证缺口的处置（二选一，勾其一）**

已合入的全部批次（至 PR #16）均先于本次修订：其合入前的 required 静态门运行带已登记存量红（`overall_status == FAIL`），M1A（R0-B）即此形态；更早的 PR #1（M0）相对其 base 已有 42 项信任根 blob 变更，任何读法下的取证均永久不可补。修订后条文（合入前取证）对这些批次追溯不可满足——本项由所有者裁定其**处置**。**无论勾哪一支，历史事实都不因签署而改变**：合入前取证缺口在 T4/T13/T19 的裁决表中一律据实记为 `UNAVAILABLE`，`overall_status == FAIL` 的历史运行据实记为 `FAIL`；**任何签署都不得把缺口改记为「证据齐全」「视为已裁决」「不计缺失」或任何等价表述**。两支的区别只在于「是否再为这些批次另做一次具名的补充重验」，不在于记录怎么写，也不在于门禁结果——两支下裁决字段与「§4.5 合入前取证证据齐全」一项的评定完全相同。

- ☑ 风险接受（`RISK_ACCEPTED`）：所有者知悉上述缺口且接受其风险，不再要求为这些历史批次补证。本签署的唯一效力，是在相应条目上附加一条平台可见、署日期、逐条列明所涉批次的风险接受记录；**它不改变裁决字段**——条目仍记 `UNAVAILABLE`，只是同时带 `RISK_ACCEPTED` 标注。**`RISK_ACCEPTED` 不满足 `PASS` 的充要条件**：审计不得因该标注把对应退出条件判为通过、不得据此写 `PASS`、不得把它当作证据引用；(c) 第 (3) 项因此不成立，M1 维持冻结。签署本支不排除所有者此后任何时点自行选择执行下一支的等价重验（那是选项、不是义务）；重验完成后在同一条目上另附具名的重验记录，`RISK_ACCEPTED` 标注保留，裁决字段仍为 `UNAVAILABLE`，该项仍计为不满足。（签署：2026-07-26）
- ☐ 要求等价重验（补充记录，无门禁效力）：所有者希望在 main 的 required 检查全部转绿后，于当时 main tip 上对各历史批次的承载用例重跑一次并留存 CI 记录，作为今日等价性的补充说明（历史记录不改写）。**该重验既不是被补上的合入前证据，也不替代它**：其对象是当时 main tip 的树与该批次的用例子集，与这些批次合入前的 head 及其门禁结果不同源，故不能证明其合入前的门禁结果。裁决表相应条目**在重验前后一律记 `UNAVAILABLE`**；重验完成后可在同一条目附 `REVALIDATED@<CI run ref>` 作为**补充记录**，该记录**不满足**硬规则 1 中「§4.5 合入前取证证据齐全」一项，不计为任何退出条件通过、不作为证据引用、不得使 `总裁决` 变为 `PASS`，也不改变 (c) 第 (3) 项的评定。**本支不产生任何合入后才到期的义务**：其前置（为 `main` 启用 required status checks 并使其全绿）与 §4.5 残余披露同一归口，属仓库设置层面工作、不属于任何施工批次的交付范围；重验做与不做、何时做均由所有者自行决定，不设时限、不依赖任何后续 PR，不做即维持现状。
