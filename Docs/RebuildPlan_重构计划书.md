...
| release-binding-composition | 两阶段互斥常绿门（稳定 required check ID）：M4 前为 `release-binding.negative-gate`——可执行证明生产 publish/dispatch 入口实际不可用（非 manifest 元数据自证）；M4 起为 `release-binding.same-instance-gate`——policy-approval 与 executor-gateway 由同一 active-binding provider 实例生产组合，activation/revocation/rollback、generation/token 一致性、restart、并发读集成测试。两门切换必须在 M4 批次同一合入提交上原子完成：任一时点 required 清单内两门必有其一存在且 PASS，不得出现两门皆缺的窗口 |
