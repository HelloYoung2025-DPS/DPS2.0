...
- **签署前须知（§4.5「残余披露」的就地交叉引用；本须知不增减本项的任何效力）**：本项签署后，触碰信任根批次据以关闭的 required 静态门 `PASS`，是**由候选自身携带的校验器签发的**——`.github/workflows/static-ci.yml` 与它执行的 `Tools/ci/run_phase0_gate.py` 本身都在 `CANDIDATE_TRUST_PATHS` 内，一个削弱校验器的改动可以让被削弱后的校验器为它自己出具 `PASS`。**该残余不随本项签署与否而改变**：本项未签署时回落的原两段式，其第 1 步与第 2 步同样由候选自身携带的代码执行，且第 2 步在本仓历史上从无 CI 执行路径。故本项签署的是「原第 2 步是否废止」，**不是**「自签发问题是否已解决」；后者的机制解法归口为仓库设置层面的配置工作（为 `main` 启用 required status checks、收紧施工凭证），不属于任何施工批次的交付范围。全文、实测依据及其与 `Docs/Operations/RepositoryProtection_仓库保护.md:21` 的正面冲突见 §4.5「残余披露」。本须知只作提示：不新增勾选项、不创设待办、不产生任何合入后才到期的义务或后续 PR 依赖。
