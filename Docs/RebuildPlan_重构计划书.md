...
接受本计划后，施工按**显式带门序列**推进（不用序数 PR 编号）：**R0-A（M0）→ R0-B（receipt 治理迁移）→ T2A（composition-gate 评估器引导发布，独立信任根批次）→ R0-C 收口（BOM 权威迁移）→ T2B（R0-D 过渡清单治理批次）→ R0-D/T3（删除 11 个 factory 模块）**。其中：R0-C 施工可在 T2A 前先行，但收口硬前置 = T2A 已交付 NEGATIVE/SAME_INSTANCE 评估器捆绑包与外部摘要锚定（§12 release-binding-composition 行）、独立外审、owner 亲合——R0-C 收口注册的 required check 组引用该锚定摘要，缺引导发布则 R0-C 不得收口；T3 开工硬前置 = T2B 的 owner 授权过渡清单已外部锚定并经 supersession 并入 m1b-closure 回执（缺则 composition-gate 对删除 diff fail-closed，序列在 T2B 处停住而非绕过）。Soul 轨和执行轨从 M1C 后并行，不提前物理删除 Legacy。
