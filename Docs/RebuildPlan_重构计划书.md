...
接受本计划后，第一个实现 PR 只做 M0/R0-A：恢复固定工具链、取得干净可比较基线、输出保留侧错误迁移表。第二个 PR 只做 receipt 治理迁移，第三个 PR 只做 BOM 权威迁移；**BOM 权威迁移批次（R0-C）收口前，须先完成"composition-gate 评估器引导发布"独立批次**——交付 NEGATIVE/SAME_INSTANCE 评估器捆绑包与外部摘要锚定（§12 release-binding-composition 行）、独立外审、owner 亲合；R0-C 收口注册的 required check 引用该已合入引导发布的锚定摘要，缺引导发布则 R0-C 不得收口。三者独立审核后，第四个 PR 才删除 11 个 factory 模块。Soul 轨和执行轨从 M1C 后并行，不提前物理删除 Legacy。
