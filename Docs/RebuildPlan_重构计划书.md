...
| M1B | 普通 candidate BOM validator、active release binding（durable truth store）、外部 signer 合同 | policy 与 gateway 按合同层同源证明读同一 generation/token（生产进程接线属 M4，见 §4.3 交付边界），回滚/撤销测试 PASS |
