...
while the ledger is load-bearing -- fails closed.
...
def unittest_failure_fingerprint(
    ...
    items: set = set()
    for index in range(ran_index):
        if lines[index] != _UNITTEST_SEPARATOR_LINE:
            continue
        if index + 1 >= ran_index:
            continue
        header = _UNITTEST_DETAIL_HEADER.match(lines[index + 1])
        if header is None:
            continue
        items.add(
            _unittest_failure_item(
...
def fingerprint_relation(
    ...
    while its executed count and missing-inventory list are unchanged.  Every
    dotnet and error-set deviation -- including a genuine partial fix, a line
    number drift after editing a file with registered errors, or a changed
    test count -- is GREW: reseeding those fingerprints is an Owner-merged
    ledger change by design, never an automatic pass.
    """
...
def _fingerprint_errors(fingerprint: Any) -> List[str]:
    ...
    if kind == "unittest":
        if set(fingerprint) != {
            ...
            return [
                "unittest fingerprint must have exactly "
                "kind/failed_tests/count/executed/missing_inventory"
            ]
        errors.extend(
...
# MF5: the executed-test floor is the pre-ledger floor of 167 plus the 75
# tests of the baseline red ledger guard suite; both constants are module
# level so the guard suite can pin itself against them.
PHASE0_MINIMUM_ADVERSARIAL_TESTS = 242
PHASE0_REQUIRED_UNITTEST_INVENTORY = frozenset(
    {
        ...
        "test_introducing_the_ledger_is_allowed",
