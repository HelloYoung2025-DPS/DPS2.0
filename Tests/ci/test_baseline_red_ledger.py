...
DEFAULT_CAUSE = "Traceback (most recent call last):\n  boom\n"
...
def unittest_log(
    ...
    ``causes`` supplies one traceback body per header (default: the same inert
    ``boom`` body for every block), so a fixture can hold the failed-test
    identities fixed while varying only the failure cause.
    """
    ...
    bodies = list(causes) if causes is not None else [DEFAULT_CAUSE] * len(headers)
    if len(bodies) != len(headers):
        raise AssertionError("one cause body per header is required")
    blocks = "".join(
        SEPARATOR
        ...
        + body
        + "\n"
        for (kind, method, qualified), body in zip(headers, bodies)
    )
    return (
        prefix
        + blocks
...
# P0-2 fixtures: one and the same test identity failing for three different
# causes, plus pure-noise restatements of the first cause.
#
# CF5-5: the frame path is no longer collapsed to its base name, so the noise
# a real CI log carries is the *checkout prefix* (stripped because the gate
# runs inside it) and the line number -- not the directory.  These fixtures
# are therefore written against the gate's own ROOT, which is what the CI log
# actually contains.
CHECKOUT = str(run_phase0_gate.ROOT).rstrip("/")
TMPROOT = tempfile.gettempdir().rstrip("/")
ASSERTION_CAUSE = (
    "Traceback (most recent call last):\n"
    '  File "' + CHECKOUT + '/Tests/ci/test_mod.py", line 41, '
    "in test_two\n"
    "    self.assertEqual(expected, observed)\n"
    "AssertionError: 'anchor-present' != 'anchor-missing'\n"
)
ASSERTION_CAUSE_MOVED = (
    "Traceback (most recent call last):\n"
    '  File "' + CHECKOUT + '/Tests/ci/test_mod.py", line 907, in test_two\n'
    "    self.assertEqual(expected, observed)  # moved far down the file\n"
    "        ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^\n"
    "AssertionError: 'anchor-present' != 'anchor-missing'\n"
)
# Same failure observed from two scratch checkouts under mkdtemp.  Only the
# random segment differs.  R6-2: the temporary-path rule is gone, so this pair
# is now a MISMATCH -- a wall-clock/tempdir difference can only cost a
# fail-closed red, and that is preferred over the collision the rule admitted.
ASSERTION_CAUSE_NOISY = (
    "Traceback (most recent call last):\n"
    '  File "' + TMPROOT + '/dps-fixture-7f2a/Tests/ci/test_mod.py", line 41, '
    "in test_two\n"
    "    self.assertEqual(expected, observed)\n"
    "AssertionError: 'anchor-present' != 'anchor-missing'\n"
)
ASSERTION_CAUSE_NOISY_OTHER_SEGMENT = (
    "Traceback (most recent call last):\n"
    '  File "' + TMPROOT + '/dps-fixture-3b91/Tests/ci/test_mod.py", line 41, '
    "in test_two\n"
    "    self.assertEqual(expected, observed)\n"
    "AssertionError: 'anchor-present' != 'anchor-missing'\n"
)
# CF5-5 red: the same base name reached through a different directory is a
# different failure site and must never share a digest.
ASSERTION_CAUSE_OTHER_DIRECTORY = (
    "Traceback (most recent call last):\n"
    '  File "' + CHECKOUT + '/Tests/other/test_mod.py", line 41, '
    "in test_two\n"
    "    self.assertEqual(expected, observed)\n"
    "AssertionError: 'anchor-present' != 'anchor-missing'\n"
)
VALUE_ERROR_CAUSE = (
    ...
    '  File "' + CHECKOUT + '/Tests/ci/test_mod.py", line 41, '
    "in test_two\n"
    '    raise ValueError("trust root signature is unverifiable")\n'
    "ValueError: trust root signature is unverifiable\n"
)
WORSE_ASSERTION_CAUSE = (
    "Traceback (most recent call last):\n"
    '  File "' + CHECKOUT + '/Tests/ci/test_mod.py", line 41, '
    "in test_two\n"
    "    self.assertEqual(expected, observed)\n"
    "AssertionError: 'anchor-present' != 'anchor-forged'\n"
)
...
class FingerprintParserTests(unittest.TestCase):
    def test_unittest_fingerprint_keeps_full_module_qualified_names(self) -> None:
        ...
        shared_digest = run_phase0_gate._unittest_reason_digest(
            DEFAULT_CAUSE.splitlines()
        )
        self.assertEqual(
            {
                "kind": "unittest",
                ...
                "failure_reasons": {
                    "test_mod.ExampleTests.test_three": shared_digest,
                    "test_mod.ExampleTests.test_two": shared_digest,
                },
                "count": 2,
                "executed": 5,
                "missing_inventory": [],
    ...
    def test_unittest_fingerprint_binds_the_failure_cause(self) -> None:
        # CF2-P0-2: identity-only fingerprints let a registered security test
        # keep failing for a brand new, more serious reason under a MATCH.
        headers = [("FAIL", "test_two", "test_mod.ExampleTests.test_two")]
        assertion = unittest_failure_fingerprint(
            unittest_log(headers, causes=[ASSERTION_CAUSE])
        )
        value_error = unittest_failure_fingerprint(
            unittest_log(headers, causes=[VALUE_ERROR_CAUSE])
        )
        worse_assertion = unittest_failure_fingerprint(
            unittest_log(headers, causes=[WORSE_ASSERTION_CAUSE])
        )
        identity = ["test_mod.ExampleTests.test_two"]
        for fingerprint in (assertion, value_error, worse_assertion):
            self.assertNotIn("parse_error", fingerprint)
            self.assertEqual(identity, fingerprint["failed_tests"])
            self.assertEqual(
                identity, sorted(fingerprint["failure_reasons"]), fingerprint
            )
        digests = {
            fingerprint["failure_reasons"]["test_mod.ExampleTests.test_two"]
            for fingerprint in (assertion, value_error, worse_assertion)
        }
        # Different exception type and different assertion message body must
        # each yield their own digest: three identities collapse to one, three
        # causes stay three.
        self.assertEqual(3, len(digests), digests)
        for digest in digests:
            self.assertRegex(digest, r"^[0-9a-f]{64}$")

    def digest_of(self, cause: str) -> str:
        headers = [("FAIL", "test_two", "test_mod.ExampleTests.test_two")]
        fingerprint = unittest_failure_fingerprint(
            unittest_log(headers, causes=[cause])
        )
        self.assertNotIn("parse_error", fingerprint, fingerprint)
        return fingerprint["failure_reasons"]["test_mod.ExampleTests.test_two"]

    def test_failure_cause_digest_normalizes_only_unstable_noise(self) -> None:
        # What survives normalization is exactly two things: the checkout
        # prefix, and the line number plus the source echo and caret anchor of
        # a frame inside the traceback preamble.  Those are the only two rules
        # that fire on the real Tests/ci red log (41 of 707 lines each).
        headers = [("FAIL", "test_two", "test_mod.ExampleTests.test_two")]
        baseline = unittest_failure_fingerprint(
            unittest_log(headers, causes=[ASSERTION_CAUSE])
        )
        drifted = unittest_failure_fingerprint(
            unittest_log(headers, causes=[ASSERTION_CAUSE_MOVED])
        )
        self.assertEqual(
            baseline["failure_reasons"],
            drifted["failure_reasons"],
            ASSERTION_CAUSE_MOVED,
        )
        self.assertEqual("MATCH", fingerprint_relation(drifted, baseline))
        # R6-2: two scratch checkouts differing only in the mkdtemp segment
        # are no longer folded together.  The temporary-path rule rewrote zero
        # lines of the real red log, so deleting it costs no measurable
        # stability, and an un-normalized temporary path can only MISMATCH.
        self.assertNotEqual(
            self.digest_of(ASSERTION_CAUSE_NOISY),
            self.digest_of(ASSERTION_CAUSE_NOISY_OTHER_SEGMENT),
        )

    def test_checkout_prefix_alias_is_the_only_path_rule_left(self) -> None:
        # The one retained path rule is the checkout prefix, in both spellings
        # of *this* checkout's root (macOS resolves /tmp/x to /private/tmp/x,
        # and a subprocess that never resolved the path prints /tmp/x).  It is
        # a literal prefix, so it can only fold two spellings of one checkout
        # together -- never two different directories.
        alias = run_phase0_gate._checkout_prefixes()
        self.assertIn(CHECKOUT, alias)
        twin = (
            CHECKOUT[len("/private") :]
            if CHECKOUT.startswith("/private/")
            else "/private" + CHECKOUT
        )
        self.assertIn(twin, alias)
        template = (
            "Traceback (most recent call last):\n"
            "AssertionError: manifest missing at {0}/Modules/alpha/module.json\n"
        )
        self.assertEqual(
            self.digest_of(template.format(CHECKOUT)),
            self.digest_of(template.format(twin)),
        )
        # A temporary path that is not this checkout is untouched, both in the
        # /private spelling and without it.
        self.assertNotEqual(
            self.digest_of(template.format("/private/tmp/other-checkout")),
            self.digest_of(template.format("/tmp/other-checkout")),
        )

    def test_frame_directory_stays_in_the_digest(self) -> None:
        # CF5-5 red: collapsing the frame path to its base name made
        # Tests/ci/test_mod.py and Tests/other/test_mod.py -- two different
        # files, two different failure sites -- share one digest.  The ROOT
        # prefix strip already removes checkout-location noise, so the
        # directory buys no stability and must stay.
        self.assertNotEqual(
            self.digest_of(ASSERTION_CAUSE),
            self.digest_of(ASSERTION_CAUSE_OTHER_DIRECTORY),
        )

    def test_every_temporary_path_segment_stays_in_the_digest(self) -> None:
        # R6-2 red: after the fifth round the rule still erased the whole
        # first segment under a temporary root, unconditionally -- so it could
        # not tell an mkdtemp segment from one the code under test chose.
        # The rule rewrote zero lines of the real red log; it is deleted, and
        # every segment of a temporary path now reaches the digest verbatim.
        template = (
            "Traceback (most recent call last):\n"
            '  File "' + CHECKOUT + '/Tests/ci/test_trust_boundary.py", line 91, '
            "in test_locked_third_party_imports_originate_in_repository_venv\n"
            "    self.assertIsNotNone(relative)\n"
            "ValueError: <pkg>/__init__.py is not in the subpath of "
            + TMPROOT
            + "/dps-fixture-7f2a/{0}\n"
        )
        benign = self.digest_of(template.format("vendored-copy.py"))
        hostile = self.digest_of(
            template.format("ATTACKER-CONTROLLED-SITE-PACKAGES/evil.py")
        )
        self.assertNotEqual(benign, hostile)
        # The random segment is no longer absorbed either.
        self.assertNotEqual(
            benign,
            self.digest_of(
                template.replace("dps-fixture-7f2a", "dps-fixture-3b91").format(
                    "vendored-copy.py"
                )
            ),
        )

    def test_temporary_root_child_without_a_tail_stays_in_the_digest(self) -> None:
        # R6-2a/b end-to-end shapes: a temporary-root child with no tail at
        # all was the exact form the fifth-round rule could not distinguish --
        # /tmp/dps-sandbox and /tmp/etc-shadow-link both became "<tmp>", and so
        # did /var/folders/trusted and /var/folders/attacker.  The fifth-round
        # guard only covered paths that carried a tail, which is why the hole
        # survived a full review round.
        sandbox = "Traceback (most recent call last):\nAssertionError: {0}\n"
        self.assertNotEqual(
            self.digest_of(sandbox.format("resolved sandbox root /tmp/dps-sandbox")),
            self.digest_of(
                sandbox.format("resolved sandbox root /tmp/etc-shadow-link")
            ),
        )
        self.assertNotEqual(
            self.digest_of(sandbox.format("state directory /var/folders/trusted")),
            self.digest_of(sandbox.format("state directory /var/folders/attacker")),
        )

    def test_every_hex_address_stays_in_the_digest(self) -> None:
        # R6-3 red: the fifth-round rule claimed to be anchored to a CPython
        # repr but required neither the opening '<' nor " object at ", so
        # "mapped at 0xDEADBEEF>" and "<PolicyFloor floor at 0xDEADBEEF>" were
        # rewritten too.  Zero hits on the real red log; deleted outright.
        template = "AssertionError: policy floor {0} != {1}\n"
        self.assertNotEqual(
            self.digest_of(
                "Traceback (most recent call last):\n"
                + template.format("0xDEADBEEF", "0xDEADBEEE")
            ),
            self.digest_of(
                "Traceback (most recent call last):\n"
                + template.format("0x00000000", "0xFFFFFFFF")
            ),
        )
        mapped = "Traceback (most recent call last):\nAssertionError: {0}\n"
        self.assertNotEqual(
            self.digest_of(mapped.format("mapped at 0xDEADBEEF> expected")),
            self.digest_of(mapped.format("mapped at 0x00000000> expected")),
        )
        self.assertNotEqual(
            self.digest_of(
                mapped.format("<PolicyFloor floor at 0xDEADBEEF> mismatch")
            ),
            self.digest_of(
                mapped.format("<PolicyFloor floor at 0x00000000> mismatch")
            ),
        )
        # Even the textbook CPython repr tail keeps its address now.
        self.assertNotEqual(
            self.digest_of(mapped.format("<Anchor object at 0x7f9c1d2e3f40> stale")),
            self.digest_of(mapped.format("<Anchor object at 0x55a10bcd0090> stale")),
        )

    def test_timestamps_stay_in_the_digest_wherever_they_appear(self) -> None:
        # R6-1 red: the fifth-round rule tried to keep timestamps inside an
        # assertion message while erasing "logger prefix" ones, but its only
        # discriminator was "followed by whitespace then a word character",
        # which INFO and "exceeds the permitted window" share.  Zero hits on
        # the real red log; deleted, so no timestamp is normalized anywhere.
        template = "AssertionError: anchor expiry {0} != {1}\n"
        self.assertNotEqual(
            self.digest_of(
                "Traceback (most recent call last):\n"
                + template.format("2026-01-01T00:00:00Z", "2026-01-02T00:00:00Z")
            ),
            self.digest_of(
                "Traceback (most recent call last):\n"
                + template.format("1970-01-01T00:00:00Z", "2099-12-31T23:59:59Z")
            ),
        )
        # R6-1a: line-leading stamp followed by the verdict word.  This is the
        # shape that released "1970-01-01T00:00:00Z exceeds the permitted
        # window" against a registered "2026-01-01T00:00:00Z ..." at exit 0.
        exceeds = (
            "Traceback (most recent call last):\n"
            "AssertionError: anchor validity rejected\n"
            "{0} exceeds the permitted window\n"
        )
        self.assertNotEqual(
            self.digest_of(exceeds.format("2026-01-01T00:00:00Z")),
            self.digest_of(exceeds.format("1970-01-01T00:00:00Z")),
        )
        # R6-1b: an indented two-line validity window.
        window = (
            "Traceback (most recent call last):\n"
            "AssertionError: anchor validity window rejected\n"
            "  {0} not_before boundary\n"
            "  {1} not_after boundary\n"
        )
        self.assertNotEqual(
            self.digest_of(
                window.format("2026-01-01T00:00:00Z", "2027-01-01T00:00:00Z")
            ),
            self.digest_of(
                window.format("1970-01-01T00:00:00Z", "2099-12-31T23:59:59Z")
            ),
        )
        # R6-1c: a bracketed stamp, the form the rule was written for.
        bracketed = (
            "Traceback (most recent call last):\n"
            "AssertionError: certificate rejected\n"
            "[{0}] not_before the pinned trust root\n"
        )
        self.assertNotEqual(
            self.digest_of(bracketed.format("2026-01-01T00:00:00Z")),
            self.digest_of(bracketed.format("1970-01-01T00:00:00Z")),
        )
        # A genuine logger prefix line is a MISMATCH now too.  It never
        # occurred in the real red log, and a MISMATCH only costs a red.
        prefix = (
            "Traceback (most recent call last):\n"
            "AssertionError: anchor rejected\n"
            "{0} INFO runner emitted the report\n"
        )
        self.assertNotEqual(
            self.digest_of(prefix.format("2026-07-25T04:05:06")),
            self.digest_of(prefix.format("2026-07-26T23:59:59")),
        )
        compared = (
            "Traceback (most recent call last):\n"
            "AssertionError: expiry mismatch\n"
            "{0} != 2026-06-06T00:00:00Z\n"
        )
        self.assertNotEqual(
            self.digest_of(compared.format("2026-01-01T00:00:00Z")),
            self.digest_of(compared.format("1970-01-01T00:00:00Z")),
        )

    def test_frame_shaped_line_in_a_message_never_swallows_its_detail(self) -> None:
        # CF5-4 red: frame swallowing ran everywhere, so an aggregated
        # sub-failure report that embeds a File "...", line N line lost every
        # indented detail beneath it -- including the whole reason.
        template = (
            "Traceback (most recent call last):\n"
            '  File "' + CHECKOUT + '/Tools/ci/graph.py", line 12, in check\n'
            "    raise AssertionError(report)\n"
            "AssertionError: aggregate sub-failure report:\n"
            '  File "Modules/alpha/graph.py", line 44, in build\n'
            "      reason: {0}\n"
        )
        self.assertNotEqual(
            self.digest_of(template.format("reciprocal edge missing for alpha")),
            self.digest_of(template.format("TRUST ROOT SIGNATURE FORGED")),
        )
        # Frames of the real traceback preamble are still normalized.
        self.assertEqual(
            self.digest_of(template.format("reciprocal edge missing for alpha")),
            self.digest_of(
                template.format("reciprocal edge missing for alpha").replace(
                    '/Tools/ci/graph.py", line 12',
                    '/Tools/ci/graph.py", line 4210',
                )
            ),
        )

    def test_inline_set_repr_order_does_not_change_the_digest(self) -> None:
        # U1: repr(set) on a single assertion line is rendered in per-process
        # hash order, so a registered entry flipped red at random.  Sorting is
        # a permutation of the items -- membership stays load bearing.
        template = "AssertionError: {0} != {{'d'}}\n"

        def digest(rendering: str) -> str:
            return self.digest_of(
                "Traceback (most recent call last):\n"
                + template.format(rendering)
            )

        self.assertEqual(
            digest("{'a', 'b', 'c'}"), digest("{'c', 'a', 'b'}")
        )
        # C5b: different membership is still a different cause.
        self.assertNotEqual(
            digest("{'a', 'b', 'c'}"), digest("{'a', 'b', 'z'}")
        )
        self.assertNotEqual(
            digest("{'a', 'b', 'c'}"), digest("{'a', 'b'}")
        )
        # Sequences keep their order: only brace groups are set shaped.
        self.assertNotEqual(
            digest("['a', 'b', 'c']"), digest("['c', 'a', 'b']")
        )
        # A dict repr is insertion ordered and never hash randomized, so its
        # order is information and stays untouched.
        self.assertNotEqual(
            digest("{'a': 1, 'b': 2}"), digest("{'b': 2, 'a': 1}")
        )
        # Only str/bytes hashing is randomized, so only a brace group made of
        # string literals may be reordered.  Prose in braces -- where the
        # order may be the whole point -- is left alone.
        self.assertNotEqual(
            digest("{root, intermediate, leaf}"),
            digest("{leaf, root, intermediate}"),
        )

    def test_stray_banner_parse_error_quotes_the_offending_lines(self) -> None:
        # U2: one decorative 70-'=' banner voids every registered entry of the
        # check, so the parse error has to carry the surrounding lines.
        headers = [("FAIL", "test_two", "test_mod.ExampleTests.test_two")]
        fingerprint = unittest_failure_fingerprint(
            unittest_log(
                headers,
                causes=[
                    "Traceback (most recent call last):\n"
                    "AssertionError: report follows\n"
                    + SEPARATOR
                    + "\nDECORATIVE BANNER FROM test_two\n"
                ],
            )
        )
        reason = fingerprint["parse_error"]
        self.assertIn("does not open a detail block", reason)
        self.assertIn("AssertionError: report follows", reason)
        self.assertIn("DECORATIVE BANNER FROM test_two", reason)

    def test_address_and_timestamp_drift_is_a_new_cause(self) -> None:
        # Same assertion, only the repr address moved: this used to be one
        # digest and is now two.  A relocated object is a MISMATCH, which
        # costs a red -- the direction the whole rule set is biased toward.
        headers = [("FAIL", "test_two", "test_mod.ExampleTests.test_two")]
        template = (
            "Traceback (most recent call last):\n"
            "AssertionError: <Anchor object at {0}> rejected at 2026-07-25\n"
        )
        first = unittest_failure_fingerprint(
            unittest_log(headers, causes=[template.format("0x7f9c1d2e3f40")])
        )
        second = unittest_failure_fingerprint(
            unittest_log(headers, causes=[template.format("0x55a10bcd0090")])
        )
        self.assertNotEqual(first["failure_reasons"], second["failure_reasons"])
        semantic = unittest_failure_fingerprint(
            unittest_log(
                headers,
                causes=[
                    "Traceback (most recent call last):\n"
                    "AssertionError: <Anchor object at 0x7f9c1d2e3f40> accepted "
                    "at 2026-07-25\n"
                ],
            )
        )
        self.assertNotEqual(first["failure_reasons"], semantic["failure_reasons"])

    def test_set_difference_item_order_does_not_change_the_digest(self) -> None:
        # Real instability observed across two independent Tests/ci runs: the
        # suite runs under -I, which suppresses PYTHONHASHSEED, so unittest
        # renders set differences in a per-process random order.  The items
        # describe a set, so their order carries no information -- but their
        # membership does.
        headers = [("FAIL", "test_two", "test_mod.ExampleTests.test_two")]

        def cause(*items: str) -> str:
            return (
                "Traceback (most recent call last):\n"
                '  File "Tests/ci/test_mod.py", line 889, in test_two\n'
        ...
        def digest(*items: str) -> str:
            fingerprint = unittest_failure_fingerprint(
                unittest_log(headers, causes=[cause(*items)])
            )
            self.assertNotIn("parse_error", fingerprint)
            return fingerprint["failure_reasons"]["test_mod.ExampleTests.test_two"]

        self.assertEqual(
            digest("'a.bin'", "'b.bin'", "'c.bin'"),
            digest("'c.bin'", "'a.bin'", "'b.bin'"),
        )
        # Membership is still load bearing: drop one, add one, both differ.
        self.assertNotEqual(
            digest("'a.bin'", "'b.bin'", "'c.bin'"), digest("'a.bin'", "'b.bin'")
        )
        self.assertNotEqual(
            digest("'a.bin'", "'b.bin'", "'c.bin'"),
            digest("'a.bin'", "'b.bin'", "'d.bin'"),
        )

    def test_ordered_lines_outside_a_set_block_keep_their_order(self) -> None:
        # Only unittest's own set-difference headers unlock sorting; ordinary
        # message lines must stay order sensitive.
        headers = [("FAIL", "test_two", "test_mod.ExampleTests.test_two")]

        def digest(first: str, second: str) -> str:
            fingerprint = unittest_failure_fingerprint(
                unittest_log(
                    headers,
                    causes=[
        ...
        self.assertNotEqual(digest("- alpha", "+ beta"), digest("+ beta", "- alpha"))

    def test_detail_block_without_a_rule_line_is_a_parse_error(self) -> None:
        forged = (
            SEPARATOR
            + "\nFAIL: test_two (test_mod.ExampleTests.test_two)\n"
            + "AssertionError: boom\n\n"
        ...
        self.assertIn("70-dash rule line", fingerprint["parse_error"])

    def test_stray_separator_cannot_truncate_a_cause(self) -> None:
        # A 70-'=' line that opens no detail block would end the previous
        # block's body early, splitting a cause in two and dropping the tail
        # from its digest.  Inside the parse domain that is unparseable.
        truncating = unittest_log(
            [("FAIL", "test_two", "test_mod.ExampleTests.test_two")],
            causes=[
                "Traceback (most recent call last):\n"
                "AssertionError: head\n" + SEPARATOR + "\ntail that would vanish\n"
            ],
        )
        fingerprint = unittest_failure_fingerprint(truncating)
        self.assertIn("parse_error", fingerprint)
        self.assertIn("does not open a detail block", fingerprint["parse_error"])

    def test_empty_detail_block_body_is_a_parse_error(self) -> None:
        forged = unittest_log(
            [("FAIL", "test_two", "test_mod.ExampleTests.test_two")], causes=[""]
        )
        fingerprint = unittest_failure_fingerprint(forged)
        self.assertIn("parse_error", fingerprint)
        self.assertIn("empty body", fingerprint["parse_error"])

    def test_same_identity_with_conflicting_causes_is_a_parse_error(self) -> None:
        base = "FAIL: test_matrix (test_mod.ExampleTests.test_matrix) (a=1)"
        log = (
            SEPARATOR + "\n" + base + "\n" + DASHES + "\n" + ASSERTION_CAUSE + "\n"
            + SEPARATOR + "\n" + base + "\n" + DASHES + "\n" + VALUE_ERROR_CAUSE
            + "\n"
        ...
        fingerprint = unittest_failure_fingerprint(log)
        self.assertIn("parse_error", fingerprint)
        self.assertIn("disagree on the failure cause", fingerprint["parse_error"])
...
class LedgerParseTests(unittest.TestCase):
    ...
    def test_wrong_schema_version_is_rejected(self) -> None:
        text = ledger_text([self.entry()]).replace(
            BASELINE_RED_LEDGER_SCHEMA_VERSION, "dps.baseline-red-ledger/v9"
        )
        with self.assertRaisesRegex(Phase0Error, "schema_version"):
            parse_baseline_red_ledger(text, "fixture")

    def test_cause_blind_v1_document_fails_closed(self) -> None:
        # The cause-bound unittest fingerprint is a breaking schema change:
        # every pre-cause (v1) document must be refused outright rather than
        # accepted as a compatible ledger.
        self.assertEqual("dps.baseline-red-ledger/v2", BASELINE_RED_LEDGER_SCHEMA_VERSION)
        legacy_entry = self.entry()
        legacy_entry["failure_fingerprint"] = {
            key: value
            for key, value in legacy_entry["failure_fingerprint"].items()
            if key != "failure_reasons"
        }
        legacy_text = ledger_text([legacy_entry]).replace(
            BASELINE_RED_LEDGER_SCHEMA_VERSION, "dps.baseline-red-ledger/v1"
        )
        with self.assertRaisesRegex(Phase0Error, "schema_version"):
            parse_baseline_red_ledger(legacy_text, "legacy document")

    def test_unittest_entry_without_failure_reasons_is_rejected(self) -> None:
        entry = self.entry()
        del entry["failure_fingerprint"]["failure_reasons"]
        with self.assertRaisesRegex(Phase0Error, "failure_reasons"):
            parse_baseline_red_ledger(ledger_text([entry]), "fixture")

    def test_failure_reasons_must_cover_exactly_the_failed_tests(self) -> None:
        entry = self.entry()
        reasons = entry["failure_fingerprint"]["failure_reasons"]
        dropped = dict(reasons)
        dropped.pop(sorted(dropped)[0])
        entry["failure_fingerprint"]["failure_reasons"] = dropped
        with self.assertRaisesRegex(Phase0Error, "failure_reasons"):
            parse_baseline_red_ledger(ledger_text([entry]), "fixture")
        entry = self.entry()
        entry["failure_fingerprint"]["failure_reasons"]["test_mod.Ghost.test_x"] = (
            "0" * 64
        )
        with self.assertRaisesRegex(Phase0Error, "failure_reasons"):
            parse_baseline_red_ledger(ledger_text([entry]), "fixture")

    def test_failure_reason_digests_must_be_sha256_hex(self) -> None:
        entry = self.entry()
        reasons = entry["failure_fingerprint"]["failure_reasons"]
        reasons[sorted(reasons)[0]] = "NOT-A-DIGEST"
        with self.assertRaisesRegex(Phase0Error, "failure_reasons"):
            parse_baseline_red_ledger(ledger_text([entry]), "fixture")
...
class ThreeStateVerdictTests(unittest.TestCase):
    ...
    def test_new_failure_reason_on_a_registered_test_is_a_mismatch(self) -> None:
        # CF2-P0-2 red at gate level: the registered check keeps exactly the
        # same failed-test set and executed count, but every one of its
        # failures now has a different, more serious cause.  The old
        # identity-only fingerprint returned MATCHED and exit 0.
        checks, entries = self.matching_fixture()
        registered_log = unittest_log(
            UNITTEST_HEADERS, causes=[ASSERTION_CAUSE, DEFAULT_CAUSE]
        )
        ...
        checks[1] = check_for(
            "phase0-adversarial-unit-tests",
            "FAIL",
            unittest_log(UNITTEST_HEADERS, causes=[VALUE_ERROR_CAUSE, DEFAULT_CAUSE]),
        )
        state = state_for(entries=entries, baseline_entries=entries)
        overall, baseline_red = run_pipeline(checks, state)
        verdicts = {
        ...
        self.assertEqual("BLOCKED", baseline_red["result"])
        self.assertEqual("FAIL", overall)
        self.assertEqual(1, gate_exit_code(overall, False))

    def test_unchanged_failure_reason_still_matches_through_the_pipeline(self) -> None:
        checks, entries = self.matching_fixture()
        registered_log = unittest_log(
            UNITTEST_HEADERS, causes=[ASSERTION_CAUSE, DEFAULT_CAUSE]
        )
        ...
        checks[1] = check_for(
            ...
            "FAIL",
            unittest_log(
                UNITTEST_HEADERS, causes=[ASSERTION_CAUSE_MOVED, DEFAULT_CAUSE]
            ),
        )
        state = state_for(entries=entries, baseline_entries=entries)
        overall, baseline_red = run_pipeline(checks, state)
...
class LedgerDriftTests(unittest.TestCase):
    ...
    def test_introducing_an_empty_ledger_is_allowed(self) -> None:
        state = state_for(entries=[], baseline_entries=None)
    ...
    def test_bootstrap_ledger_cannot_buy_its_own_exit_zero(self) -> None:
        # CF2-P0-1 red: with no ledger at the authoritative base commit the
        # gate used to accept the candidate's self-reported entries and then
        # match them against the candidate's own current failures, so the very
        # PR that introduces the ledger could register the reds its own diff
        # created and exit 0.  Introducing entries is now indistinguishable
        # from expansion: it always fails the introducing run.
        state = state_for(entries=self.entries(), baseline_entries=None)
        self.assertEqual("LEDGER_INTRODUCED", state["drift_status"])
        self.assertNotEqual([], state["drift_errors"])
        self.assertIn("bootstrap", " ".join(state["drift_errors"]))
        check = baseline_red_ledger_drift_check(state)
        self.assertEqual("FAIL", check["status"])

    def test_bootstrap_ledger_blocks_even_when_every_entry_matches(self) -> None:
        checks = [
            check_for("clean-checkout-evidence-boundary", "PASS"),
            check_for("phase0-adversarial-unit-tests", "FAIL", UNITTEST_LOG),
            check_for("manifest:example:example.unit", "FAIL", DOTNET_COMPILE_LOG),
    ...
    def test_bootstrap_and_expansion_share_one_rule(self) -> None:
        # No ordering trick buys the bypass: adding entries fails whether the
        # base has no ledger at all or an empty one.
        entries = self.entries()
        from_absent = state_for(entries=entries, baseline_entries=None)
        from_empty = state_for(entries=entries, baseline_entries=[])
        self.assertEqual("LEDGER_INTRODUCED", from_absent["drift_status"])
        self.assertEqual("LEDGER_EXPANDED", from_empty["drift_status"])
        for state in (from_absent, from_empty):
            self.assertEqual(
    ...
    def test_unreadable_baseline_commit_is_never_read_as_a_bootstrap(self) -> None:
        # ``git cat-file -e <commit>:<path>`` cannot tell "the file is not in
        # that commit" from "that commit is not in this checkout".  Reading the
        # second as the first would classify a broken object store as a fresh
        # ledger introduction and report it to the Owner as such.
        calls: List[List[str]] = []

        def fake_run_command(command, root, timeout_seconds=None, env=None):
            calls.append(list(command))
            return phase0_module.CommandResult(list(command), 128, 0, "")

        root = Path(tempfile.mkdtemp(prefix="ledger-baseline-commit-"))
        self.addCleanup(shutil.rmtree, root, ignore_errors=True)
        (root / "governance").mkdir()
        (root / BASELINE_RED_LEDGER_RELATIVE_PATH).write_text(
            ledger_text([]), encoding="utf-8"
        )
        with mock.patch.object(run_phase0_gate, "run_command", fake_run_command):
            state = run_phase0_gate.load_baseline_red_ledger_state(root, COMMIT)
        self.assertIn(COMMIT + "^{commit}", " ".join(calls[0]))
        self.assertEqual("LEDGER_INVALID", state["drift_status"])
        self.assertIn("not readable in this checkout", " ".join(state["errors"]))
        self.assertEqual(
            "FAIL", baseline_red_ledger_drift_check(state)["status"]
        )
...
class FingerprintRelationTests(unittest.TestCase):
    ...
    def test_same_test_failing_for_a_new_reason_is_growth(self) -> None:
        # CF2-P0-2 red: identity, executed count and inventory are all held
        # fixed; only the cause changes.  Under the identity-only relation this
        # was MATCH, so a registered trust test could swap AssertionError for a
        # brand new ValueError and still buy exit 0.
        headers = [
            ("FAIL", "test_two", "test_mod.ExampleTests.test_two"),
            ("ERROR", "test_three", "test_mod.ExampleTests.test_three"),
        ]
        registered = unittest_failure_fingerprint(
            unittest_log(headers, causes=[ASSERTION_CAUSE, DEFAULT_CAUSE])
        )
        self.assertEqual("MATCH", fingerprint_relation(registered, registered))
        for replacement in (VALUE_ERROR_CAUSE, WORSE_ASSERTION_CAUSE):
            observed = unittest_failure_fingerprint(
                unittest_log(headers, causes=[replacement, DEFAULT_CAUSE])
            )
            self.assertEqual(
                registered["failed_tests"], observed["failed_tests"]
            )
            self.assertEqual(registered["executed"], observed["executed"])
            self.assertEqual("GREW", fingerprint_relation(observed, registered))

    def test_shrink_with_a_new_reason_on_the_survivor_is_growth(self) -> None:
        headers = [
            ("FAIL", "test_two", "test_mod.ExampleTests.test_two"),
            ("ERROR", "test_three", "test_mod.ExampleTests.test_three"),
        ]
        ...
        honest_shrink = unittest_failure_fingerprint(
            unittest_log(headers[:1], causes=[ASSERTION_CAUSE])
        )
        self.assertEqual("SHRUNK", fingerprint_relation(honest_shrink, registered))
        laundered_shrink = unittest_failure_fingerprint(
            unittest_log(headers[:1], causes=[VALUE_ERROR_CAUSE])
        )
        self.assertEqual("GREW", fingerprint_relation(laundered_shrink, registered))

    def test_cause_blind_fingerprint_can_never_be_a_match(self) -> None:
        # Fail closed on the pre-cause schema: a fingerprint that carries no
        # failure_reasons map is never MATCH/SHRUNK, whichever side it is on.
        registered = unittest_failure_fingerprint(
            unittest_log(
                [("FAIL", "test_two", "test_mod.ExampleTests.test_two")],
                causes=[ASSERTION_CAUSE],
            )
        )
        legacy = {
            key: value
        ...
        self.assertEqual("GREW", fingerprint_relation(legacy, registered))
        self.assertEqual("GREW", fingerprint_relation(registered, legacy))
        self.assertEqual("GREW", fingerprint_relation(legacy, legacy))
...
class TrustRootPinTests(unittest.TestCase):
    ...
    def test_normalization_prose_matches_the_surviving_rules(self) -> None:
        # R6-4: the fifth-round docstring and ledger note described rules the
        # code did not implement -- they promised that a timestamp inside an
        # assertion message survived while the regex erased it, and the guard
        # suite pinned the erasure as correct.  Prose and code are bound here.
        for attribute in (
            "_LOG_PREFIX_TIMESTAMP",
            "_ISO_TIMESTAMP_BODY",
            "_REPR_HEX_ADDRESS",
            "_temporary_path_roots",
        ):
            self.assertFalse(
                hasattr(run_phase0_gate, attribute),
                "{0} must stay deleted, not resurrected".format(attribute),
            )
        docstring = run_phase0_gate.__doc__ or ""
        self.assertIn("no timestamp is ever erased", docstring)
        self.assertIn("no temporary path is\n  ever erased", docstring)
        self.assertIn("no\n  hex address is ever erased", docstring)
        # R8: the sixth-round "exactly three rewrites / nothing else /
        # preserved verbatim" arithmetic was itself an overclaim -- measured
        # against the code, _unittest_reason_digest performs seven rewrites.
        # The prose must enumerate all seven and never resurrect the count
        # of three or the verbatim promise.
        self.assertNotIn("exactly three things", docstring)
        self.assertNotIn("nothing else", docstring)
        digest_doc = run_phase0_gate._unittest_reason_digest.__doc__ or ""
        self.assertNotIn("preserved verbatim", digest_doc)
        self.assertIn("exactly seven\n    rewrites", digest_doc)
        for numbered in ("(1)", "(2)", "(3)", "(4)", "(5)", "(6)", "(7)"):
            self.assertIn(numbered, digest_doc, numbered)
            self.assertIn(numbered, docstring, numbered)
        ledger = json.loads(
            (ROOT / BASELINE_RED_LEDGER_RELATIVE_PATH).read_text(encoding="utf-8")
        )
        notes = "\n".join(ledger["notes"])
        self.assertIn("WHAT A unittest MATCH MEANS", notes)
        self.assertIn("WHAT IS NO LONGER NORMALIZED", notes)
        self.assertNotIn("Exactly three things are erased", notes)
        for surviving in (
            "seven rewrites _unittest_reason_digest actually performs",
            "Every line loses its trailing whitespace",
            "unbounded run of lines indented deeper than the frame",
            "backslashes rewritten to forward slashes",
            "stripped from every line, in both spellings",
            "sorted only when every item is a string literal",
            "leading and trailing blank lines are dropped",
            "a distinct rule from (5), with no string-literal restriction",
            "Anything those seven rewrites do not cover is hashed as-is",
        ):
            self.assertIn(surviving, notes, surviving)
        # R8: the zero-false-red measurement is scoped to one interpreter
        # build on one runner image; stdlib frame paths sit inside the
        # digest, so an interpreter upgrade MISMATCHes a registered unittest
        # red -- fail-closed, it blocks and never releases.  With zero
        # unittest entries registered today, nothing is exposed to it yet.
        self.assertIn(
            "same interpreter build on the same runner image", notes
        )
        self.assertIn("can only block a run, never release one", notes)
        self.assertIn(
            "zero unittest entries, so no registered red is exposed", notes
        )
        # The note must not describe any rule that no longer exists.
        for retired in (
            "the random segment of a temporary directory",
            "'<... at 0x...>' repr addresses",
            "wall-clock timestamps in the line-leading logger position",
        ):
