# DPS v4.5 代码规范 (conventions_代码规范.md)
...
| **元组解构** | `(a, b) = GetPair()` | 分别赋值 |
| **throw 表达式** | `var x = y ?? throw new ...` | `if-throw` 语句块 |
| **^ 索引** | `arr[^1]` | `arr[arr.Length - 1]` |
| **.. 范围** | `arr[1..3]` | 手动切片 |

> **编译错误识别**: 使用禁止语法时编译器会报错，例如:
> `[模块名] 编译错误 行42: Feature '字符串插值' is not available in C# 5. Please use language version 6 or greater.`
> 遇到此类错误，对照上表替换语法即可。

### ZDProjects/*.cs 层：C# 7.0+
ZennoDroid 的 "OwnCode" 脚本层支持较新语法，允许使用：
