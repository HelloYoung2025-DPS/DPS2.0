...
Do not store task plans, progress records, or gate verdicts in the repository. A versioned expected-failure baseline that the required gate consumes as input is not a verdict; changing it requires its own reviewed pull request. Record audit and governance verdicts in repository issues; use the task, issue, or pull-request description for temporary planning. Do not open zero-diff or marker pull requests solely to carry a verdict.
