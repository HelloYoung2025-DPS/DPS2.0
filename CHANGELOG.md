...
- 外审规则经用户裁定更正（同日生效，取代上一条中的"第二票"定位）：**开发期批次合入外审 = Codex 一票 + required 门禁 + 用户批准**；计划书 §9.1 的双异构复核判归项目运行时无人值守安全网（交付物），DeepSeek 脚本降级为 advisory 预备资产——其判决不构成合入条件，外审机制文档不再维护其 SHA-256 指纹（接线批次自行定版）。脚本本体保留敏感扫描（含 `--focus` 参数）与"显式 FAIL 不折算 PASS"语义。新增半自动驾驶授权（外审机制 §一之三）与驱动会话模板 TA；并按 Codex round-2 外审加固该机制的三处安全/逻辑漏洞：T18/LC Legacy 物理删除排除自动合并（§11 独立批准）、自动合入用 `--match-head-commit <已审SHA>` 绑定已审 head（防 PASS 后新推送被裸 merge）、WP 探针（T15）显式前置于 T10 破死锁。
