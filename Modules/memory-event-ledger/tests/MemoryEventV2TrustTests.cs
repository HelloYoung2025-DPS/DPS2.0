...
public sealed class MemoryEventV2TrustTests
{
    ...
    public async Task FixedAuthoritiesPrepareExactCanonicalEvent()
    {
        ...
        using (ledger) using (signer)
        {
            ...
            var prepared = await ledger.PrepareAsync(V2TestData.AppendRequest(signer, request, signals));
    ...
    public async Task PublicV1BooleanAndSoulDtoCannotAppend()
    {
        ...
        var exception = await Assert.ThrowsAsync<NotSupportedException>(() => legacy.AppendAsync(soul, TestData.Event(soul: soul)));
    ...
    public async Task ForgedSoulRawBytesOrCrossScopeFailsClosed()
    {
        ...
        using (ledger) using (signer)
        {
            ...
            await Assert.ThrowsAsync<MemoryCapabilityException>(() =>
                ledger.PrepareAsync(V2TestData.AppendRequest(signer, request, signals)));

            source.Mutate = null;
            var other = V2TestData.Request('b', request.EventId);
            await Assert.ThrowsAsync<MemoryCapabilityException>(() =>
                ledger.PrepareAsync(V2TestData.AppendRequest(signer, request, signals, V2TestData.SignedReceipt(signer, other, signals, other.OccurredAt))));
        }
    }
    ...
    public async Task RawSwapWrongKeyUnknownOutcomeAndActedCodeFailClosed()
    {
        ...
        using (ledger) using (signer)
        {
            ...
            await Assert.ThrowsAnyAsync<Exception>(() => ledger.PrepareAsync(V2TestData.AppendRequest(signer, request, signals, raw)));

            using var wrongSigner = ECDsa.Create(ECCurve.NamedCurves.nistP256);
            await Assert.ThrowsAsync<MemoryCapabilityException>(() =>
                ledger.PrepareAsync(V2TestData.AppendRequest(signer, request, signals, V2TestData.SignedReceipt(wrongSigner, request, signals, request.OccurredAt))));
            await Assert.ThrowsAsync<MemoryCapabilityException>(() =>
                ledger.PrepareAsync(V2TestData.AppendRequest(signer, request, signals, V2TestData.SignedReceipt(signer, request, signals, request.OccurredAt, outcome: "UNKNOWN_OUTCOME", nativeVerified: false, postconditionVerified: false, nativeEvidence: null, postconditionEvidence: null))));
            await Assert.ThrowsAsync<MemoryCapabilityException>(() =>
                ledger.PrepareAsync(V2TestData.AppendRequest(signer, request, signals, V2TestData.SignedReceipt(signer, request, signals, request.OccurredAt, resultCode: "ACTED_VERIFIED"))));
            ...
            await Assert.ThrowsAsync<MemoryCapabilityException>(() =>
                ledger.PrepareAsync(V2TestData.AppendRequest(
                    signer, replayEvent, signals, V2TestData.SignedReceipt(signer, request, signals, request.OccurredAt))));
        }
    }
    ...
    public async Task EqualExpiryAndStaleReceiptFailClosed(int minutes)
    {
        ...
        using (ledger) using (signer)
        {
            ...
            await Assert.ThrowsAsync<MemoryCapabilityException>(() => ledger.PrepareAsync(V2TestData.AppendRequest(
                signer, request, signals, V2TestData.SignedReceipt(signer, request, signals, V2TestData.Now.AddMinutes(minutes)))));
        }
    }
    ...
    public async Task RevokedSoulAndExpiredPreparedCapabilityFailClosed()
    {
        ...
        using (ledger) using (signer)
        {
            ...
            await Assert.ThrowsAsync<MemoryCapabilityException>(() => ledger.AppendAsync(prepared));
            resultState.RevocationEpoch--;
    ...
    public async Task RecomputedOrJsonRoundTrippedPreparedObjectCannotAuthorizeAppend()
    {
        ...
        using (ledger) using (signer)
        {
            ...
            await Assert.ThrowsAsync<MemoryCapabilityException>(() => ledger.AppendAsync(recomputed));
