...
- Added the `control-plane-host.same-instance-integration` required suite (4 REAL_POSTGRESQL cases): one shared `ActiveReleaseBindingAuthority` instance over one durable `PostgresReleaseBindingTruthStore` is injected as the composition-fixed `IActiveReleaseBindingReader` into both the real executor adapter (`ControlPlaneHostActiveReleaseBomReader`) and the real production policy recovery composition (`PolicyApprovalSubmissionRecoveryClient`), proving both consumer paths observe the same generation/token/status across activation, revocation, rollback, restart replay, and foreign-device fail-closed — an engineering/integration-level proof per RebuildPlan §4.3 (PR#6 v2), not a production-topology claim (M4).
