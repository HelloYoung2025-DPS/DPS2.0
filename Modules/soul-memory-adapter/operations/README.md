...
`SoftDeleteProjectionAsync` accepts only a `gbrain.projection.delete-intent/v1` envelope containing the exact Source/Soul/device/account, trace, operation idempotency key, mutation time, expected revision, and expected checksum. `RebuildProjectionAsync` likewise requires a separate `gbrain.projection.rebuild-intent/v1` operation envelope. A scope-only delete, an equal/older operation time, a target mismatch, a reused key, a lost fence, or an unresolved prior dispatch fails closed. If an old delete is replayed after a rebuild or newer write, the active page is retained.
