// =====================================================
// Initializer_OwnCode.cs - ZD Own Code 入口
...
Func<string, string, object[], object> RunModule = (filePath, methodName, args) => {
    ...
    if (t == null) { project.SendErrorToLog("[Initializer] 找不到类: " + className); return null; }
    var method = t.GetMethod(methodName); if (method == null) { project.SendErrorToLog("[Initializer] 找不到方法: " + methodName); return null; }
...
try {
    ...
    if (string.IsNullOrEmpty(root)) { project.SendErrorToLog("[Initializer] project_root 未设置"); return "ERROR: project_root 未设置"; }
    if (!root.EndsWith("\\")) root += "\\";
    string modulePath = root + "Modules\\Initializer.cs";
    project.SendInfoToLog("[Initializer] 加载模块: " + modulePath);
    object result = RunModule(modulePath, "Run", new object[] { project });
    return result != null ? result.ToString() : "ERROR: 模块执行失败";
}
catch (System.Exception ex) { project.SendErrorToLog("[Initializer] 异常: " + ex.Message); return "ERROR: " + ex.Message; }
