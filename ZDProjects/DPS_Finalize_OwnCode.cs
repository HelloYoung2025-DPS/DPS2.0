// =====================================================
// DPS_Finalize_OwnCode.cs - ZD Own Code 入口（会话终结）
// v4.7.0: ZD 外层流程编排 - 最后一步
// 复制此代码到 ZennoDroid 的 C# code 动作块
// 调用 SessionRunner.FinalizeSession() 完成会话收尾
...
Func<string, string, object[], object> RunModule = (filePath, methodName, args) => {
    if (!System.IO.File.Exists(filePath)) {
        project.SendErrorToLog("[DPS_Finalize] 模块不存在: " + filePath);
        return null;
    }
    ...
    if (result.Errors.HasErrors) {
        foreach (System.CodeDom.Compiler.CompilerError e in result.Errors) {
            project.SendErrorToLog(string.Format("[DPS_Finalize] 编译错误 行{0}: {1}", e.Line, e.ErrorText));
        }
        return null;
    }
    ...
    if (t == null) {
        project.SendErrorToLog("[DPS_Finalize] 找不到类: " + className);
        return null;
    }
    
    var method = t.GetMethod(methodName);
    if (method == null) {
        project.SendErrorToLog("[DPS_Finalize] 找不到方法: " + methodName);
        return null;
    }
...
try {
    ...
    if (string.IsNullOrEmpty(root)) {
        project.SendErrorToLog("[DPS_Finalize] project_root 未设置");
        return "ERROR: project_root 未设置";
    }
    if (!root.EndsWith("\\")) root += "\\";
    
    string modulePath = root + "Modules\\SessionRunner.cs";
    project.SendInfoToLog("[DPS_Finalize] 加载模块: " + modulePath);
    
    // 调用 SessionRunner.FinalizeSession(project, instance)
    // 会话结束时调用，保存记忆、清理、统计、门控判定
    object result = RunModule(modulePath, "FinalizeSession", new object[] { project, instance });
    
    string resultStr = result != null ? result.ToString() : "ERROR: 模块执行失败";
    
    // 写入 ZD 变量
    project.Variables["session_result"].Value = resultStr;
    
    return resultStr;
}
catch (System.Exception ex) {
    project.SendErrorToLog("[DPS_Finalize] 异常: " + ex.Message);
    project.Variables["session_result"].Value = "ERROR: " + ex.Message;
