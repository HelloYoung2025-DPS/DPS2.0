// =====================================================
// Extension_OwnCode.cs - ZD Own Code 入口
...
Func<string, string, object[], object> RunModule = (filePath, methodName, args) => {
    if (!System.IO.File.Exists(filePath)) {
        project.SendErrorToLog("[Extension] 模块不存在: " + filePath);
        return null;
    }
    ...
    foreach (string ef in engineFiles) {
        ...
        if (System.IO.File.Exists(efPath)) { allCodes.Add(System.IO.File.ReadAllText(efPath, System.Text.Encoding.UTF8)); }
    }
    ...
    foreach (string cf in coreFiles) {
        ...
        if (System.IO.File.Exists(cfPath)) { allCodes.Add(System.IO.File.ReadAllText(cfPath, System.Text.Encoding.UTF8)); }
    }
    ...
    if (System.IO.Directory.Exists(extBaseDir)) {
        foreach (string extFile in System.IO.Directory.GetFiles(extBaseDir, "*.cs", System.IO.SearchOption.AllDirectories)) {
    ...
    foreach (string peerFile in System.IO.Directory.GetFiles(moduleDir, "*.cs", System.IO.SearchOption.TopDirectoryOnly)) {
        if (System.IO.Path.GetFileName(peerFile) != targetFileName) {
    ...
    foreach (string fileCode in allCodes) {
        foreach (string line in fileCode.Split(new string[] { "\r\n", "\n" }, System.StringSplitOptions.None)) {
            string trimmed = line.Trim();
            if (trimmed.StartsWith("using ") && trimmed.EndsWith(";") && !trimmed.Contains("(")) { usings.Add(trimmed); }
            else { codeBody.AppendLine(line); }
        }
    }
    
    var finalCode = new System.Text.StringBuilder();
    foreach (string u in usings) { finalCode.AppendLine(u); }
    ...
    if (result.Errors.HasErrors) {
        foreach (System.CodeDom.Compiler.CompilerError e in result.Errors) {
            project.SendErrorToLog(string.Format("[Extension] 编译错误 行{0}: {1}", e.Line, e.ErrorText));
        }
        return null;
    }
    ...
    if (t == null) { project.SendErrorToLog("[Extension] 找不到类: " + className); return null; }
    var method = t.GetMethod(methodName);
    if (method == null) { project.SendErrorToLog("[Extension] 找不到方法: " + methodName); return null; }
...
try {
    ...
    if (string.IsNullOrEmpty(root)) {
        project.SendErrorToLog("[Extension] project_root 未设置");
        return "ERROR: project_root 未设置";
    }
    if (!root.EndsWith("\\")) root += "\\";
    
    string modulePath = root + "Modules\\Extension.cs";
    project.SendInfoToLog("[Extension] 加载模块: " + modulePath);
    
    object result = RunModule(modulePath, "Run", new object[] { project });
    return result != null ? result.ToString() : "ERROR: 模块执行失败";
}
catch (System.Exception ex) {
    project.SendErrorToLog("[Extension] 异常: " + ex.Message);
