// =====================================================
// Maintenance_OwnCode.cs - ZD Own Code 入口
...
Func<string, string, object[], object> RunModule = (filePath, methodName, args) => {
    if (!System.IO.File.Exists(filePath)) { project.SendErrorToLog("[Maintenance] 模块不存在: " + filePath); return null; }
    ...
    if (result.Errors.HasErrors) { foreach (System.CodeDom.Compiler.CompilerError e in result.Errors) { project.SendErrorToLog(string.Format("[Maintenance] 编译错误 行{0}: {1}", e.Line, e.ErrorText)); } return null; }
    ...
    if (t == null) { project.SendErrorToLog("[Maintenance] 找不到类: " + className); return null; }
    var method = t.GetMethod(methodName); if (method == null) { project.SendErrorToLog("[Maintenance] 找不到方法: " + methodName); return null; }
...
try {
    ...
    if (string.IsNullOrEmpty(root)) { project.SendErrorToLog("[Maintenance] project_root 未设置"); return "ERROR: project_root 未设置"; }
    if (!root.EndsWith("\\")) root += "\\";
    string modulePath = root + "Modules\\Maintenance.cs";
    project.SendInfoToLog("[Maintenance] 加载模块: " + modulePath);
    object result = RunModule(modulePath, "Run", new object[] { project });
    return result != null ? result.ToString() : "ERROR: 模块执行失败";
}
catch (System.Exception ex) { project.SendErrorToLog("[Maintenance] 异常: " + ex.Message); return "ERROR: " + ex.Message; }
